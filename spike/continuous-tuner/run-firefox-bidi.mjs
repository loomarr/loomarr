...
const srv=http.createServer((q,r)=>{const f=path.join(here,q.url==='/'?'splice.html':q.url.split('?')[0]);fs.readFile(f,(e,d)=>{if(e){r.writeHead(404);r.end();return}r.writeHead(200,{'content-type':f.endsWith('.html')?'text/html':'video/mp4'});r.end(d)})}).listen(0);
