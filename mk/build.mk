...
dev-be: rust-dev-build ## backend with live reload (Air) — rebuilds + restarts on Go/Rust changes
	...
	@eval "$$(./scripts/dev-env.sh export)"; \
	  mkdir -p .agent-data "$$LOOMARR_ARTIFACT_DIR" "$${LOOMARR_AGENT_FILLER_DIR:-.filler-drop}" "$${LOOMARR_AGENT_IMAGES_DIR:-.agent-data/images}" "$${LOOMARR_AGENT_PREPARED_DIR:-.agent-data/prepared}"; \
