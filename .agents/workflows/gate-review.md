...
**2. Give the brief to an independent reviewer with fresh context.**

This matters more than anything else here. The reviewer must receive the artifact and the
...
Use the current harness's delegation facility when it has one. Otherwise save the brief for a second
agent session or human reviewer and mark the independent review pending; do not silently substitute a
self-review while calling it independent.

Brief the reviewer as:
...
**3. Report the reviewer's findings unedited**, then add your own response separately.
