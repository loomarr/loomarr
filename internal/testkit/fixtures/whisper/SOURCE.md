...
Regenerate by rebuilding the image and re-running the arguments above. The connected backend is
verified against the same audio through the OpenAI-compatible multipart `/audio/transcriptions`
contract; its response must carry a detected language and at least one timed, non-empty segment.
