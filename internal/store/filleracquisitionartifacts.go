...
// AcquisitionArtifactForClip resolves ownership from either the intended watch path or the
// catalog hash bound before intake moves the file. Newest wins only to tolerate an operator
// deliberately reusing a filename across separate acquisition runs.
func (s *sqlStore) AcquisitionArtifactForClip(
	...
	artifact, err := scanAcquisitionArtifact(s.db.QueryRowContext(ctx, s.ph(acquisitionArtifactSelect+`
		WHERE media_path = ? OR (? <> '' AND clip_hash = ?)
		ORDER BY updated_at DESC, id DESC LIMIT 1`), mediaPath, clipHash, clipHash))
