...
func testSplitProposals(t *testing.T, newStore NewStoreFunc) {
	...
	p := filler.SplitProposal{
		...
		Source: source, Structure: &structure, StructureDecision: &structureDecision, ExcludedByLanguage: 1,
	...
	if got.ClipHash != p.ClipHash || got.Source != p.Source || !reflect.DeepEqual(got.Structure, p.Structure) || !reflect.DeepEqual(got.StructureDecision, p.StructureDecision) || got.ExcludedByLanguage != 1 || len(got.Segments) != 3 || !got.CreatedAt.Equal(now) {
...
func testFillerAppliedAdmissionTransaction(t *testing.T, newStore NewStoreFunc) {
	...
	if err != nil || reviews.Total != 1 || len(reviews.Rows) != 1 ||
		!decisionPageContains(reviews, decision.ID) {
	...
	if err != nil || reviews.Total != 1 || len(reviews.Rows) != 1 ||
		!decisionPageContains(reviews, rejectedDecision.ID) {
	...
	if err := s.CommitAppliedFillerDecisionAction(ctx, unclassifiedAction); !errors.Is(err, fillerdecision.ErrActionStale) {
