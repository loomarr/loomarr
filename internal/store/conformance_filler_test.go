...
func testFillerAcquisitionArtifacts(t *testing.T, newStore NewStoreFunc) {
	...
	artifacts := []filler.AcquisitionArtifact{
		{
			...
			Provider: "archive", SourceURL: "https://archive.org/details/one", RemoteID: "one",
	...
	if remoteStates[(filler.RemoteIdentity{Provider: "archive", SourceID: run.SourceID, RemoteID: "one"}).Key()] != filler.RemoteCatalogued ||
