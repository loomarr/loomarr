-- +goose Up
-- Administrator admission decisions, hashed bearer grants, and owner-generic contact addresses
-- (§11). The grant table intentionally has no plaintext token or rendered-link column.
CREATE TABLE invitations (
    ...
    CHECK (
        (kind = 'local' AND username <> '' AND library_user_id = '' AND display_name = '' AND identity_key = lower(trim(username))) OR
...
-- Contact ownership widens from an allowlisted person to either a person or a pending Invitation.
-- Polymorphic owner existence is enforced by the store transaction; normalized uniqueness remains
-- structural across every owner and lifecycle state.
