-- +goose Up
-- Durable, provider-neutral notification work (§11). Domain references and narrow template data
-- are retained; rendered bodies, credentials, and plaintext bearer URLs have no columns here.
CREATE TABLE notification_intents (
    id                TEXT PRIMARY KEY,
    topic             TEXT NOT NULL CHECK (topic IN ('account_invitation', 'local_password_recovery')),
    recipient_kind    TEXT NOT NULL CHECK (recipient_kind IN ('invitation', 'person')),
    recipient_id      TEXT NOT NULL,
    reference_kind    TEXT NOT NULL CHECK (reference_kind IN ('invitation', 'recovery')),
...
CREATE TABLE notification_delivery_attempts (
    ...
    means                 TEXT NOT NULL CHECK (means IN ('email')),
