-- +goose Up
-- Expand the durable notification vocabulary without editing the applied account-delivery schema.
-- SQLite cannot alter CHECK constraints in place, so rebuild both linked tables transactionally.
...
CREATE TABLE notification_intents (
    ...
    reference_kind    TEXT NOT NULL CHECK (reference_kind IN ('invitation', 'recovery', 'proposal', 'title', 'channel')),
...
INSERT INTO notification_intents
    SELECT * FROM notification_intents_old;
INSERT INTO notification_delivery_attempts
    SELECT * FROM notification_delivery_attempts_old;
