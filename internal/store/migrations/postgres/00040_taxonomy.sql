...
--
--   clip_tags  the denormalised clip↔taxon rows. A clip tagged `beer` gets THREE rows — beer(leaf=t),
--              alcohol(leaf=f), drinks(leaf=f) — so a curation query `WHERE taxon='food'` is one index
