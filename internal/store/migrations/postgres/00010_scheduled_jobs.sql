...
-- concurrent replica won't re-run it (FOR UPDATE SKIP LOCKED). A "Run now" sets next_run <=
-- now. Times are epoch seconds, matching the rest of the store.
--
-- Forward-only (§16). Tenth migration; scheduled_jobs holds only regenerable state.
