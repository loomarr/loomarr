...
func TestLive_PreparedMPEGTSTailSeeksContainDecodableVideo(t *testing.T) {
	bin := ffmpegBin(t)
	dir := t.TempDir()
	sourcePath := filepath.Join(dir, "source.mp4")
	generate := []string{
	...
	for _, tc := range []struct {
		name        string
		end, offset time.Duration
	}{
		{"file tail", 24 * time.Second, 23 * time.Second},
		{"trimmed airing", 10 * time.Second, 9 * time.Second},
		{"unaligned trim", 9700 * time.Millisecond, 9100 * time.Millisecond},
	} {
		t.Run(tc.name, func(t *testing.T) {
			startedAt := time.Now().UTC().Add(-tc.offset)
			origin := newPreparedOrigin(lib, fixedPreparedResolver{ok: true, window: PreparedWindow{Current: PreparedAiring{
				Specification: spec, StartedAt: startedAt, Offset: tc.offset,
				Identity: AiringIdentity{StartedAt: startedAt, EndsAt: startedAt.Add(tc.end), Kind: schedule.SlotProgram, ContentID: "tail-source", ScheduleBlockID: "tail-block"},
			}}})
			ctx, cancel := context.WithTimeout(t.Context(), 5*time.Second)
			defer cancel()
			block, err := origin.MPEGTSBlockSource(bin, nil, nil)(ctx, BlockRequest{ChannelID: "channel", Plan: PlanFull, TimelineOrigin: time.Now().Add(-10 * time.Second)})
			if err != nil {
				t.Fatal(err)
			}
			...
			if err != nil || len(frame) != 320*180*3 {
				t.Fatalf("current airing tail has no decoded frame: bytes=%d err=%v", len(frame), err)
			}
		})
	}
}
