...
func TestCopyStartProbeBoundsFailureAndRetainsValidProof(t *testing.T) {
	for _, tc := range []struct {
		...
		{"valid", `printf '%s' '{"streams":[{"index":0,"codec_type":"video"}],"packets":[{"stream_index":0,"pts_time":"0","flags":"K_"}],"format":{"start_time":"0"}}'`, true},
		{"deadline", "exec sleep 10", false},
		{"output bound", `printf '%s' '{"streams":[{"index":0,"codec_type":"video"}],"packets":[{"stream_index":0,"pts_time":"0","flags":"K_"}],"format":{"start_time":"0"}}'; printf '%1048576s' ' '`, false},
	} {
