...
func TestRunOpenRouterPublishesExhaustiveEvidenceBoundReview(t *testing.T) {
	...
	defer server.Close()
	fixture := newReviewFixture(t, server.URL)
	runtime := fixture.runtime(server.Client(), server.URL)
...
func TestIndependentModelReviewsFeedAuthorityLock(t *testing.T) {
	...
	defer server.Close()
	first := newReviewFixture(t, server.URL)
	if _, err := runOpenRouter(t.Context(), first.config, first.runtime(server.Client(), server.URL)); err != nil {
		t.Fatal(err)
	}
	...
	secondSnapshot := fixtureSnapshotForModel(
		server.URL, first.now.Add(-time.Hour), "vendor/reviewer-model-two", "vendor/reviewer-model-two-2026",
	)
	...
	if _, err := runOpenRouter(t.Context(), second.config, second.runtime(server.Client(), server.URL)); err != nil {
		t.Fatal(err)
	}
...
func TestRunOpenRouterRejectsSourceDriftBeforeHTTP(t *testing.T) {
	...
	if _, err := runOpenRouter(t.Context(), fixture.config, fixture.runtime(server.Client(), server.URL)); err == nil ||
