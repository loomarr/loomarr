...
func TestRunOpenRouterAcceptsExactParticipantProcessorSchedule(t *testing.T) {
	...
	installKnownScriptRights(t, &fixture, server.URL, nil, func(processor *fillersafetycorpus.KnownScriptHostedProcessor) {})

	result, err := runOpenRouter(t.Context(), fixture.config, fixture.runtime(server.Client(), server.URL))
	if err != nil {
		t.Fatal(err)
	}
...
func TestRunOpenRouterRechecksParticipantConsentBeforeEveryRequest(t *testing.T) {
	...
	installKnownScriptRights(t, &fixture, server.URL, &expiresAt, func(processor *fillersafetycorpus.KnownScriptHostedProcessor) {})
	runtime := fixture.runtime(server.Client(), server.URL)
