...
func loadInputs(config Config) (loadedInputs, error) {
	plan, planRaw, err := readPrivateJSON[Plan](config.PlanPath, maximumPlanBytes)
	if err != nil {
		return loadedInputs{}, fmt.Errorf("read spoken-safety model review plan: %w", err)
	}
