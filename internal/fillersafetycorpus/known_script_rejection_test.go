...
func TestPrepareKnownScriptRejectsConsentFailuresBeforeMedia(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if fixture.wrapper.calls != 0 || fixture.wrapper.identityCalls != 0 {
				t.Fatalf("media touched before consent rejection: %d/%d", fixture.wrapper.calls, fixture.wrapper.identityCalls)
			}
		})
	}
}
...
func TestPrepareKnownScriptRejectsEvidenceAndMappingDriftBeforeMedia(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if fixture.wrapper.calls != 0 || fixture.wrapper.identityCalls != 0 {
				t.Fatalf("media touched before evidence rejection: %d/%d", fixture.wrapper.calls, fixture.wrapper.identityCalls)
			}
		})
	}
}
...
func TestPrepareKnownScriptRejectsUnauthorizedProcessorScheduleBeforeMedia(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if fixture.wrapper.calls != 0 || fixture.wrapper.identityCalls != 0 {
				t.Fatalf("media touched before processor rejection: %d/%d", fixture.wrapper.calls, fixture.wrapper.identityCalls)
			}
		})
	}
}
...
func TestPrepareKnownScriptRejectsTransformationAndAssetRightsDriftBeforeMedia(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if fixture.wrapper.calls != 0 || fixture.wrapper.identityCalls != 0 {
				t.Fatalf("media touched before transformation rejection: %d/%d", fixture.wrapper.calls, fixture.wrapper.identityCalls)
			}
		})
	}
}
...
func TestPrepareKnownScriptRejectsUnsafeAndSymlinkedSourcesBeforeMedia(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if fixture.wrapper.calls != 0 || fixture.wrapper.identityCalls != 0 {
				t.Fatalf("media touched before source rejection: %d/%d", fixture.wrapper.calls, fixture.wrapper.identityCalls)
			}
		})
	}
}
...
func TestPrepareKnownScriptCleansPartialStageAndRejectsIdentityDrift(t *testing.T) {
	...
	for _, test := range []struct {
		name   string
		mutate func(*fakeWrapper)
	}{
		{name: "wrap failure", mutate: func(wrapper *fakeWrapper) { wrapper.failAt = 3 }},
		{name: "tool identity", mutate: func(wrapper *fakeWrapper) { wrapper.driftIdentity = true }},
	} {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			fixture := newKnownScriptFixture(t)
			test.mutate(fixture.wrapper)
			if _, err := prepareKnownScript(t.Context(), fixture.config, fixture.wrapper); err == nil {
...
func TestPrepareKnownScriptRejectsEvidenceChangedDuringWrapping(t *testing.T) {
	...
	fixture.wrapper.onWrap = func(call int) {
		if call == 1 {
