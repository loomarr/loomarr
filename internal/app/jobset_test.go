...
//
// This pins the whole set, including operator groups, through the API an operator actually sees.
// Consolidating tasks therefore requires an explicit review of this list rather than silently
// dropping a registration while moving composition code.
//
...
func TestJobSet(t *testing.T) {
	want := []string{
		...
		"filler-split-sweep | filler | 0 45 4 * * * | job.filler_split_sweep.schedule",
		"filler-sync | filler | 0 */15 * * * * | job.filler_sync.schedule",
		"housekeeping | system | 0 30 4 * * * | job.housekeeping.schedule",
		// ⚠ The image service (§22, V52). `images-fetch` is the SECOND job to reach out to
		...
		//
		// ⚠ Artwork maintenance is the second task that DELETES unattended. It removes image rows nothing
		...
		"images-adopt-artwork | artwork | 0 */5 * * * * | job.images_adopt_artwork.schedule",
		"images-avif | artwork | 0 20 * * * * | job.images_avif.schedule",
		"images-fetch | artwork | 0 * * * * * | job.images_fetch.schedule",
	...
	for _, j := range body.Jobs {
		got = append(got, j.Name+" | "+j.Group+" | "+j.Schedule+" | "+j.ScheduleKey)
	}
	sort.Strings(got)
