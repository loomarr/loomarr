...
func comparePriorExposure(ctx context.Context, report *Report, candidates map[string]fingerprint, prior map[string]fingerprint) error {
	incomplete := false
	for _, source := range report.PriorSources {
		if !source.Available || (!source.Fingerprint.VisualComparable && !source.Fingerprint.AudioComparable) {
			incomplete = true
			break
		}
	}
