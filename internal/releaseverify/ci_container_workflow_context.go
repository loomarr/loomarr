...
func workflowJobContextAuthorityEntries() map[workflowJobContextKey]workflowJobContextAuthority {
	return map[workflowJobContextKey]workflowJobContextAuthority{
		...
		{workflow: "ci-expo-android-tv.yml", job: "run"}:          {name: "Expo Android TV — native build", runsOn: "ubuntu-latest"},
		...
		{workflow: "ci.yml", job: "ci-ok"}: {
			...
			needsList: []string{"changes", "release-candidate-scope", "full-manual-scope", "ci-policy", "agent-harness-macos", "rust-contracts", "go-contracts", "image-certification", "go", "store-postgres", "frontend", "clients", "apple-mobile", "apple-tv", "apple-cache-validation", "expo-android-mobile", "expo-android-tv", "playwright", "tuner", "image", "docs", "android"},
		},
