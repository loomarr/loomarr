...
func workflowJobContextAuthorityEntries() map[workflowJobContextKey]workflowJobContextAuthority {
	return map[workflowJobContextKey]workflowJobContextAuthority{
		...
		{workflow: "ci-go.yml", job: "run"}:                  {name: "Go — race-policy tests (${{ matrix.lane }})", runsOn: "ubuntu-latest", timeoutMinutes: 15, strategy: &workflowStrategyAuthority{lanes: []string{"1/4", "2/4", "3/4", "4/4", "certification-1/2", "certification-2/2"}}},
