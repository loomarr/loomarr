...
func workflowJobContextAuthorityEntries() map[workflowJobContextKey]workflowJobContextAuthority {
	return map[workflowJobContextKey]workflowJobContextAuthority{
		...
		{workflow: "ci-go.yml", job: "run"}:                  {name: "Go — race-policy tests (${{ matrix.lane }})", runsOn: "ubuntu-latest", timeoutMinutes: 15, strategy: &workflowStrategyAuthority{lanes: []string{"1/6", "2/6", "3/6", "4/6", "5/6", "6/6", "certification-1/2", "certification-2/2"}}},
		...
		{workflow: "codeql.yml", job: "analyze-actions"}:               {name: "CodeQL — Actions", runsOn: "ubuntu-latest", needs: "changes"},
		{workflow: "codeql.yml", job: "analyze-go"}:                    {name: "CodeQL — Go", runsOn: "ubuntu-latest", needs: "changes"},
		{workflow: "codeql.yml", job: "analyze-javascript-typescript"}: {name: "CodeQL — JavaScript/TypeScript", runsOn: "ubuntu-latest", needs: "changes"},
		{workflow: "codeql.yml", job: "analyze-python"}:                {name: "CodeQL — Python", runsOn: "ubuntu-latest", needs: "changes"},
		{workflow: "codeql.yml", job: "analyze-ruby"}:                  {name: "CodeQL — Ruby", runsOn: "ubuntu-latest", needs: "changes"},
		{workflow: "codeql.yml", job: "analyze-rust"}:                  {name: "CodeQL — Rust", runsOn: "ubuntu-latest", needs: "changes"},
