...
func workflowActionAuthorityEntries() map[workflowActionKey]string {
	return map[workflowActionKey]string{
		...
		{workflow: "android-beta.yml", job: "release", step: 2}:             `{"uses":"actions/setup-java@dd06d9cba3e5552c54d9f8ea23572deb30010f7c","with":{"distribution":"temurin","java-version":"21"}}`,
		{workflow: "android-beta.yml", job: "release", step: 3}:             `{"uses":"android-actions/setup-android@40fd30fb8d7440372e1316f5d1809ec01dcd3699"}`,
		{workflow: "android-beta.yml", job: "release", step: 4}:             `{"name":"Cache Gradle","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"android-release-${{ runner.os }}-${{ hashFiles('android/**/*.gradle.kts', 'android/gradle/wrapper/gradle-wrapper.properties') }}","restore-keys":"android-release-${{ runner.os }}-"}}`,
		{workflow: "android-beta.yml", job: "release", step: 8}:             `{"name":"Retain the signed AAB and release evidence","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"loomarr-android-tv-${{ inputs.version_name }}","path":".artifacts/android-release/*","if-no-files-found":"error","retention-days":30}}`,
		{workflow: "apple-compilation-cache.yml", job: "publish", step: 0}:  `{"uses":"actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1"}`,
		{workflow: "apple-compilation-cache.yml", job: "publish", step: 1}:  `{"uses":"actions/setup-node@820762786026740c76f36085b0efc47a31fe5020","with":{"node-version":"${{ env.NODE_VERSION }}"}}`,
		{workflow: "apple-compilation-cache.yml", job: "publish", step: 3}:  `{"name":"Restore pnpm and CocoaPods work","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.local/share/pnpm/store\n~/Library/Caches/CocoaPods\n","key":"apple-cache-publisher-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}-${{ github.run_id }}","restore-keys":"apple-clients-mobile-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}-"}}`,
		{workflow: "apple-compilation-cache.yml", job: "publish", step: 6}:  `{"name":"Restore the latest validated generation","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"${{ runner.temp }}/apple-compilation-cache.tar.zst","key":"${{ steps.apple-toolchain.outputs.fingerprint }}-${{ github.run_id }}","restore-keys":"${{ steps.apple-toolchain.outputs.fingerprint }}-"}}`,
		{workflow: "apple-compilation-cache.yml", job: "publish", step: 10}: `{"name":"Publish the validated generation","uses":"actions/cache/save@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"${{ runner.temp }}/apple-compilation-cache.tar.zst","key":"${{ steps.apple-toolchain.outputs.fingerprint }}-${{ github.run_id }}"}}`,
		...
		{workflow: "ci-android.yml", job: "run", step: 3}:                   `{"name":"Cache Gradle","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"gradle-${{ runner.os }}-${{ hashFiles('android/**/*.gradle.kts', 'android/gradle/wrapper/gradle-wrapper.properties') }}","restore-keys":"gradle-${{ runner.os }}-"}}`,
		...
		{workflow: "ci-apple-mobile.yml", job: "run", step: 3}:              `{"name":"Restore pnpm and CocoaPods work","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.local/share/pnpm/store\n~/Library/Caches/CocoaPods\n","key":"apple-clients-mobile-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}","restore-keys":"apple-clients-mobile-${{ runner.os }}-"}}`,
		{workflow: "ci-apple-mobile.yml", job: "run", step: 6}:              `{"name":"Restore the validated Apple compilation cache","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"${{ runner.temp }}/apple-compilation-cache.tar.zst","key":"${{ steps.apple-toolchain.outputs.fingerprint }}-${{ github.run_id }}","restore-keys":"${{ steps.apple-toolchain.outputs.fingerprint }}-"}}`,
		{workflow: "ci-apple-mobile.yml", job: "run", step: 8}:              `{"name":"Restore ExpoModulesJSI native slice","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"web/node_modules/.pnpm/expo-modules-jsi@*/node_modules/expo-modules-jsi/apple/Products","key":"apple-jsi-mobile-${{ runner.arch }}-${{ steps.apple-toolchain.outputs.fingerprint }}-${{ hashFiles('web/pnpm-lock.yaml') }}","restore-keys":"apple-jsi-mobile-${{ runner.arch }}-${{ steps.apple-toolchain.outputs.fingerprint }}-"}}`,
		{workflow: "ci-apple-mobile.yml", job: "run", step: 10}:             `{"name":"Keep simulator screenshot","if":"always()","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"apple-client-mobile","path":"${{ runner.temp }}/apple-client-mobile/","if-no-files-found":"warn","retention-days":7}}`,
		...
		{workflow: "ci-apple-tv.yml", job: "run", step: 3}:                  `{"name":"Restore pnpm and CocoaPods work","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.local/share/pnpm/store\n~/Library/Caches/CocoaPods\n","key":"apple-clients-tv-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}","restore-keys":"apple-clients-tv-${{ runner.os }}-"}}`,
		{workflow: "ci-apple-tv.yml", job: "run", step: 6}:                  `{"name":"Restore the validated Apple compilation cache","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"${{ runner.temp }}/apple-compilation-cache.tar.zst","key":"${{ steps.apple-toolchain.outputs.fingerprint }}-${{ github.run_id }}","restore-keys":"${{ steps.apple-toolchain.outputs.fingerprint }}-"}}`,
		{workflow: "ci-apple-tv.yml", job: "run", step: 8}:                  `{"name":"Restore ExpoModulesJSI native slice","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"web/node_modules/.pnpm/expo-modules-jsi@*/node_modules/expo-modules-jsi/apple/Products","key":"apple-jsi-tv-${{ runner.arch }}-${{ steps.apple-toolchain.outputs.fingerprint }}-${{ hashFiles('web/pnpm-lock.yaml') }}","restore-keys":"apple-jsi-tv-${{ runner.arch }}-${{ steps.apple-toolchain.outputs.fingerprint }}-"}}`,
		{workflow: "ci-apple-tv.yml", job: "run", step: 10}:                 `{"name":"Keep simulator screenshot","if":"always()","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"apple-client-tv","path":"${{ runner.temp }}/apple-client-tv/","if-no-files-found":"warn","retention-days":7}}`,
		...
		{workflow: "ci-go-contracts.yml", job: "run", step: 12}:          `{"name":"Save contract caches (main only)","if":"always() && github.ref == 'refs/heads/main'","uses":"actions/cache/save@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/go/pkg/mod\n~/.cache/go-build\n~/.cache/golangci-lint\n","key":"${{ steps.cache.outputs.cache-primary-key }}"}}`,
