...
func workflowActionAuthorityEntries() map[workflowActionKey]string {
	entries := map[workflowActionKey]string{
		{workflow: "android-ccache-promotion.yml", job: "promote", step: 0}: `{"uses":"actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1"}`,
		{workflow: "android-ccache-promotion.yml", job: "promote", step: 2}: `{"name":"Download the immutable compiler-cache transfer","if":"steps.producer.outputs.found == 'true'","uses":"actions/download-artifact@3e5f45b2cfb9172054b4087a40e8e0b5a5461e7c","with":{"name":"${{ steps.producer.outputs.artifact-name }}","path":"${{ runner.temp }}/loomarr-android-ccache-transfer","github-token":"${{ secrets.GITHUB_TOKEN }}","repository":"${{ github.repository }}","run-id":"${{ steps.producer.outputs.run-id }}"}}`,
		{workflow: "android-ccache-promotion.yml", job: "promote", step: 4}: `{"name":"Publish the verified generation in the default-branch cache scope","if":"steps.producer.outputs.found == 'true'","uses":"actions/cache/save@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"${{ runner.temp }}/loomarr-android-ccache","key":"${{ steps.cache.outputs.cache-key }}"}}`,
		...
		{workflow: "ci-android.yml", job: "run", step: 8}:                   `{"name":"Restore ccache compiler results","id":"ccache-cache","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"${{ runner.temp }}/loomarr-android-ccache","key":"android-tv-ccache-v2-${{ runner.os }}-ccache-4.14-${{ hashFiles('web/apps/tv/**', 'web/packages/**', 'web/pnpm-lock.yaml', 'web/scripts/**') }}-${{ github.run_id }}","restore-keys":"android-tv-ccache-v2-${{ runner.os }}-ccache-4.14-${{ hashFiles('web/apps/tv/**', 'web/packages/**', 'web/pnpm-lock.yaml', 'web/scripts/**') }}-\nandroid-tv-ccache-v2-${{ runner.os }}-ccache-4.14-\n"}}`,
		{workflow: "ci-android.yml", job: "run", step: 9}:                   `{"name":"Restore Gradle","id":"gradle-cache","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"android-tv-react-native-v1-${{ runner.os }}-temurin-21-node-${{ env.NODE_VERSION }}-${{ hashFiles('web/apps/tv/**', 'web/packages/**', 'web/pnpm-lock.yaml', 'web/scripts/**') }}-${{ github.sha }}-${{ github.run_id }}","restore-keys":"android-tv-react-native-v1-${{ runner.os }}-temurin-21-node-${{ env.NODE_VERSION }}-${{ hashFiles('web/apps/tv/**', 'web/packages/**', 'web/pnpm-lock.yaml', 'web/scripts/**') }}-${{ github.sha }}-"}}`,
		{workflow: "ci-android.yml", job: "run", step: 12}:                  `{"name":"Save ccache for later pull-request revisions","if":"github.event_name == 'pull_request'","uses":"actions/cache/save@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"${{ runner.temp }}/loomarr-android-ccache","key":"${{ steps.ccache-cache.outputs.cache-primary-key }}"}}`,
		{workflow: "ci-android.yml", job: "run", step: 13}:                  `{"name":"Save Gradle for later pull-request revisions","if":"github.event_name == 'pull_request'","uses":"actions/cache/save@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"${{ steps.gradle-cache.outputs.cache-primary-key }}"}}`,
		{workflow: "ci-android.yml", job: "run", step: 15}:                  `{"name":"Transfer the verified cache for trusted main publication","if":"github.event_name == 'merge_group'","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"loomarr-android-ccache-${{ github.sha }}-${{ github.run_id }}","path":"${{ runner.temp }}/loomarr-android-ccache-transfer/","if-no-files-found":"error","include-hidden-files":true,"compression-level":0,"retention-days":1}}`,
		{workflow: "ci-android.yml", job: "run", step: 16}:                  `{"name":"Retain the exact unsigned merge-result bundle","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"loomarr-android-unsigned-${{ github.sha }}","path":".artifacts/android-ci/*","if-no-files-found":"error","retention-days":30}}`,
		{workflow: "ci-android.yml", job: "run", step: 17}:                  `{"name":"Retain Android build diagnostics separately from promotion artifacts","if":"${{ always() && steps.android.outcome != 'skipped' }}","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"loomarr-android-profile-${{ github.sha }}-${{ github.run_id }}-${{ github.run_attempt }}","path":".artifacts/android-build-profile/","if-no-files-found":"error","retention-days":30}}`,
		...
		{workflow: "ci-go.yml", job: "run", step: 4}:                     `{"name":"Restore Go test caches (modules + build)","id":"gocache","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/go/pkg/mod\n~/.cache/go-build\n","key":"go-test-${{ runner.os }}-${{ steps.epoch.outputs.week }}-${{ matrix.lane }}-${{ hashFiles('go.sum') }}-${{ github.run_id }}","restore-keys":"go-test-${{ runner.os }}-${{ steps.epoch.outputs.week }}-${{ matrix.lane }}-${{ hashFiles('go.sum') }}-\ngo-test-${{ runner.os }}-${{ steps.epoch.outputs.week }}-${{ matrix.lane }}-\n"}}`,
		{workflow: "ci-go.yml", job: "run", step: 6}:                     `{"name":"Cache production FFmpeg archive","id":"ffmpeg-cache","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.cache/loomarr-ffmpeg","key":"ffmpeg-${{ runner.os }}-${{ runner.arch }}-${{ steps.ffmpeg-pin.outputs.sha256 }}"}}`,
