...
func workflowActionAuthorityEntries() map[workflowActionKey]string {
	return map[workflowActionKey]string{
		...
		{workflow: "android-beta.yml", job: "release", step: 4}:             `{"uses":"actions/setup-node@820762786026740c76f36085b0efc47a31fe5020","if":"inputs.renderer == 'react-native'","with":{"node-version":"22"}}`,
		{workflow: "android-beta.yml", job: "release", step: 6}:             `{"name":"Cache Gradle","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"android-release-${{ inputs.renderer }}-${{ runner.os }}-${{ hashFiles('android/**/*.gradle.kts', 'android/gradle/wrapper/gradle-wrapper.properties', 'web/apps/tv/**', 'web/packages/**', 'web/scripts/**', 'web/pnpm-lock.yaml') }}","restore-keys":"android-release-${{ inputs.renderer }}-${{ runner.os }}-"}}`,
		{workflow: "android-beta.yml", job: "release", step: 11}:            `{"name":"Retain the signed AAB and release evidence","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"loomarr-android-tv-${{ inputs.version_name }}","path":".artifacts/android-release/*","if-no-files-found":"error","retention-days":30}}`,
		...
		{workflow: "ci-expo-android-mobile.yml", job: "run", step: 3}:       `{"uses":"actions/setup-node@820762786026740c76f36085b0efc47a31fe5020","with":{"node-version":"${{ env.NODE_VERSION }}"}}`,
		{workflow: "ci-expo-android-mobile.yml", job: "run", step: 5}:       `{"name":"Cache pnpm store","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.local/share/pnpm/store","key":"pnpm-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}","restore-keys":"pnpm-${{ runner.os }}-"}}`,
		{workflow: "ci-expo-android-mobile.yml", job: "run", step: 6}:       `{"name":"Restore Expo Android mobile native work","id":"gradle-cache","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"expo-android-mobile-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}-${{ github.run_id }}","restore-keys":"expo-android-mobile-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}-\nexpo-android-mobile-${{ runner.os }}-\n"}}`,
		{workflow: "ci-expo-android-mobile.yml", job: "run", step: 9}:       `{"name":"Keep the standalone mobile APK","if":"always()","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"expo-android-mobile","path":"web/apps/mobile/android/app/build/outputs/apk/debug/app-debug.apk","if-no-files-found":"error","retention-days":7}}`,
		{workflow: "ci-expo-android-mobile.yml", job: "run", step: 10}:      `{"name":"Save Expo Android mobile native work (main only)","if":"always() && github.ref == 'refs/heads/main'","uses":"actions/cache/save@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"${{ steps.gradle-cache.outputs.cache-primary-key }}"}}`,
		{workflow: "ci-expo-android-tv.yml", job: "run", step: 0}:           `{"uses":"actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1"}`,
		{workflow: "ci-expo-android-tv.yml", job: "run", step: 1}:           `{"uses":"actions/setup-java@dd06d9cba3e5552c54d9f8ea23572deb30010f7c","with":{"distribution":"temurin","java-version":"21"}}`,
		{workflow: "ci-expo-android-tv.yml", job: "run", step: 2}:           `{"uses":"android-actions/setup-android@40fd30fb8d7440372e1316f5d1809ec01dcd3699"}`,
		{workflow: "ci-expo-android-tv.yml", job: "run", step: 3}:           `{"uses":"actions/setup-node@820762786026740c76f36085b0efc47a31fe5020","with":{"node-version":"${{ env.NODE_VERSION }}"}}`,
		{workflow: "ci-expo-android-tv.yml", job: "run", step: 5}:           `{"name":"Cache pnpm store","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.local/share/pnpm/store","key":"pnpm-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}","restore-keys":"pnpm-${{ runner.os }}-"}}`,
		{workflow: "ci-expo-android-tv.yml", job: "run", step: 6}:           `{"name":"Restore Expo Android TV native work","id":"gradle-cache","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"expo-android-tv-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}-${{ github.run_id }}","restore-keys":"expo-android-tv-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}-\nexpo-android-tv-${{ runner.os }}-\n"}}`,
		{workflow: "ci-expo-android-tv.yml", job: "run", step: 9}:           `{"name":"Keep the standalone TV APK","if":"always()","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"expo-android-tv","path":"web/apps/tv/android/app/build/outputs/apk/debug/app-debug.apk","if-no-files-found":"error","retention-days":7}}`,
		{workflow: "ci-expo-android-tv.yml", job: "run", step: 10}:          `{"name":"Save Expo Android TV native work (main only)","if":"always() && github.ref == 'refs/heads/main'","uses":"actions/cache/save@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"${{ steps.gradle-cache.outputs.cache-primary-key }}"}}`,
