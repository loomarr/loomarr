...
func workflowActionAuthorityEntries() map[workflowActionKey]string {
	return map[workflowActionKey]string{
		...
		{workflow: "android-beta.yml", job: "release", step: 4}:             `{"uses":"actions/setup-node@820762786026740c76f36085b0efc47a31fe5020","with":{"node-version":"22"}}`,
		{workflow: "android-beta.yml", job: "release", step: 8}:             `{"name":"Cache Gradle","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"android-release-${{ runner.os }}-${{ hashFiles('web/pnpm-lock.yaml') }}-${{ github.run_id }}","restore-keys":"android-release-${{ runner.os }}-"}}`,
		{workflow: "android-beta.yml", job: "release", step: 12}:            `{"name":"Retain the signed AAB and release evidence","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"loomarr-android-tv-${{ inputs.version_name }}","path":".artifacts/android-release/*","if-no-files-found":"error","retention-days":30}}`,
