...
func workflowActionAuthorityEntries() map[workflowActionKey]string {
	return map[workflowActionKey]string{
		...
		{workflow: "ci-android.yml", job: "run", step: 9}:                   `{"name":"Retain the exact unsigned merge-result bundle","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"loomarr-android-unsigned-${{ github.sha }}","path":".artifacts/android-ci/*","if-no-files-found":"error","retention-days":30}}`,
