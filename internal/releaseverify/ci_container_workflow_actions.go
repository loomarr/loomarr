...
func workflowActionAuthorityEntries() map[workflowActionKey]string {
	return map[workflowActionKey]string{
		...
		{workflow: "android-beta.yml", job: "release", step: 2}:          `{"uses":"actions/setup-java@c5195efecf7bdfc987ee8bae7a71cb8b11521c00","with":{"distribution":"temurin","java-version":"21"}}`,
		{workflow: "android-beta.yml", job: "release", step: 3}:          `{"uses":"android-actions/setup-android@9fc6c4e9069bf8d3d10b2204b1fb8f6ef7065407"}`,
		...
		{workflow: "android-beta.yml", job: "release", step: 8}:          `{"name":"Retain the signed AAB and release evidence","uses":"actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02","with":{"name":"loomarr-android-tv-${{ inputs.version_name }}","path":".artifacts/android-release/*","if-no-files-found":"error","retention-days":30}}`,
		...
		{workflow: "ci-android.yml", job: "run", step: 1}:                `{"uses":"actions/setup-java@c5195efecf7bdfc987ee8bae7a71cb8b11521c00","with":{"distribution":"temurin","java-version":"21"}}`,
		{workflow: "ci-android.yml", job: "run", step: 2}:                `{"uses":"android-actions/setup-android@9fc6c4e9069bf8d3d10b2204b1fb8f6ef7065407"}`,
		...
		{workflow: "ci-apple-mobile.yml", job: "run", step: 6}:           `{"name":"Cache ExpoModulesJSI native slice","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"web/node_modules/.pnpm/expo-modules-jsi@*/node_modules/expo-modules-jsi/apple/Products","key":"apple-jsi-mobile-${{ runner.arch }}-${{ steps.apple-toolchain.outputs.fingerprint }}-${{ hashFiles('web/pnpm-lock.yaml') }}","restore-keys":"apple-jsi-mobile-${{ runner.arch }}-${{ steps.apple-toolchain.outputs.fingerprint }}-"}}`,
		...
		{workflow: "ci-apple-tv.yml", job: "run", step: 6}:               `{"name":"Cache ExpoModulesJSI native slice","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"web/node_modules/.pnpm/expo-modules-jsi@*/node_modules/expo-modules-jsi/apple/Products","key":"apple-jsi-tv-${{ runner.arch }}-${{ steps.apple-toolchain.outputs.fingerprint }}-${{ hashFiles('web/pnpm-lock.yaml') }}","restore-keys":"apple-jsi-tv-${{ runner.arch }}-${{ steps.apple-toolchain.outputs.fingerprint }}-"}}`,
		...
		{workflow: "ci-go-contracts.yml", job: "run", step: 3}:           `{"name":"Restore contract caches","id":"cache","uses":"actions/cache/restore@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/go/pkg/mod\n~/.cache/go-build\n~/.cache/golangci-lint\n","key":"go-contracts-${{ runner.os }}-${{ steps.epoch.outputs.week }}-v2.12.2-${{ hashFiles('go.sum', 'Cargo.lock') }}-${{ github.run_id }}","restore-keys":"go-contracts-${{ runner.os }}-${{ steps.epoch.outputs.week }}-v2.12.2-${{ hashFiles('go.sum', 'Cargo.lock') }}-\ngo-contracts-${{ runner.os }}-${{ steps.epoch.outputs.week }}-v2.12.2-\n"}}`,
		...
		{workflow: "ci-image.yml", job: "run", step: 1}:                  `{"uses":"docker/setup-buildx-action@bb05f3f5519dd87d3ba754cc423b652a5edd6d2c"}`,
		...
		{workflow: "release-notes.yml", job: "publish-notes", step: 0}:   `{"name":"Check out the tagged commit","uses":"actions/checkout@11bd71901bbe5b1630ceea73d27597364c9af683"}`,
		...
		{workflow: "release.yml", job: "build", step: 2}:                 `{"uses":"docker/setup-buildx-action@bb05f3f5519dd87d3ba754cc423b652a5edd6d2c"}`,
		...
		{workflow: "release.yml", job: "build", step: 6}:                 `{"name":"Upload the digest","uses":"actions/upload-artifact@ea165f8d65b6e75b540449e92b4886f43607fa02","with":{"name":"release-digest-${{ matrix.arch }}","path":"/tmp/digests/${{ matrix.arch }}","if-no-files-found":"error","retention-days":1}}`,
		{workflow: "release.yml", job: "publish", step: 0}:               `{"uses":"actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1"}`,
		{workflow: "release.yml", job: "publish", step: 2}:               `{"uses":"docker/setup-buildx-action@bb05f3f5519dd87d3ba754cc423b652a5edd6d2c"}`,
		...
		{workflow: "release.yml", job: "publish", step: 4}:               `{"name":"Download the per-arch digests","uses":"actions/download-artifact@d3f86a106a0bac45b974a628896c90dbdf5c8093","with":{"path":"/tmp/digests","pattern":"release-digest-*","merge-multiple":true}}`,
