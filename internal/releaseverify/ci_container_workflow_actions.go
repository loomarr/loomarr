...
func workflowActionAuthorityEntries() map[workflowActionKey]string {
	entries := map[workflowActionKey]string{
		...
		{workflow: "ci-android.yml", job: "run", step: 8}:                   `{"name":"Cache ccache compiler results","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"${{ runner.temp }}/loomarr-android-ccache","key":"android-tv-ccache-v1-${{ runner.os }}-ccache-4.14-${{ hashFiles('web/apps/tv/**', 'web/packages/**', 'web/pnpm-lock.yaml', 'web/scripts/**') }}","restore-keys":"android-tv-ccache-v1-${{ runner.os }}-ccache-4.14-"}}`,
		{workflow: "ci-android.yml", job: "run", step: 9}:                   `{"name":"Cache Gradle","id":"gradle-cache","uses":"actions/cache@55cc8345863c7cc4c66a329aec7e433d2d1c52a9","with":{"path":"~/.gradle/caches\n~/.gradle/wrapper\n","key":"android-tv-react-native-v1-${{ runner.os }}-temurin-21-node-${{ env.NODE_VERSION }}-${{ hashFiles('web/apps/tv/**', 'web/packages/**', 'web/pnpm-lock.yaml', 'web/scripts/**') }}-${{ github.sha }}-${{ github.run_id }}","restore-keys":"android-tv-react-native-v1-${{ runner.os }}-temurin-21-node-${{ env.NODE_VERSION }}-${{ hashFiles('web/apps/tv/**', 'web/packages/**', 'web/pnpm-lock.yaml', 'web/scripts/**') }}-${{ github.sha }}-"}}`,
		{workflow: "ci-android.yml", job: "run", step: 12}:                  `{"name":"Retain the exact unsigned merge-result bundle","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"loomarr-android-unsigned-${{ github.sha }}","path":".artifacts/android-ci/*","if-no-files-found":"error","retention-days":30}}`,
		{workflow: "ci-android.yml", job: "run", step: 13}:                  `{"name":"Retain Android build diagnostics separately from promotion artifacts","if":"${{ always() && steps.android.outcome != 'skipped' }}","uses":"actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a","with":{"name":"loomarr-android-profile-${{ github.sha }}-${{ github.run_id }}-${{ github.run_attempt }}","path":".artifacts/android-build-profile/","if-no-files-found":"error","retention-days":30}}`,
