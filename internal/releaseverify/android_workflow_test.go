...
func TestVerifyAndroidReleaseWorkflow(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "renderer defaults to rollback", mutate: replaceOnce("        default: compose", "        default: react-native", t), wantErr: true},
		{name: "unknown renderer is rejected", mutate: replaceOnce("          - react-native", "          - react-native\n          - other", t), wantErr: true},
		...
		{name: "React Native install stays conditional", mutate: replaceOnce("      - name: Install the React Native workspace\n        if: inputs.renderer == 'react-native'", "      - name: Install the React Native workspace\n        if: always()", t), wantErr: true},
		{name: "adoption gate cannot be removed", mutate: replaceOnce("        run: test \"$REACT_NATIVE_ADOPTED\" = true", "        run: true", t), wantErr: true},
		{name: "adoption gate cannot be bypassed", mutate: replaceOnce("        if: inputs.renderer == 'react-native' && inputs.publish_to_play", "        if: inputs.publish_to_play", t), wantErr: true},
