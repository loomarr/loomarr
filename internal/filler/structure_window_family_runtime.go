...
func (r *StructureWindowFamilyRuntime) Assess(ctx context.Context, prepared StructureAssessmentWindowMediaSet) (fillerstructurewindow.StitchResult, error) {
	if r == nil || r.assessor == nil || r.evidence == nil || fillerstructure.ValidateAssessorProfile(r.profile) != nil {
		return fillerstructurewindow.StitchResult{}, errors.New("structure window family runtime is unavailable")
	}
	...
	if prepared.Authority.Plan.Source != wantSource || validatePreparedStructureAssessmentWindows(prepared) != nil {
		return fillerstructurewindow.StitchResult{}, errors.New("structure window family runtime media authority is invalid")
	}
	...
	for ordinal, window := range prepared.Windows {
		...
		if err != nil {
			return fillerstructurewindow.StitchResult{}, fmt.Errorf("find structure window %d evidence: %w", ordinal, err)
		}
		if !found {
			recorded, err = r.assessor.AssessWindow(ctx, prepared.Authority, window)
			if err != nil {
				return fillerstructurewindow.StitchResult{}, fmt.Errorf("structure window %d produced no authority: %w", ordinal, err)
			}
		}
		if !validRecordedWindowAuthority(prepared.Authority, window, r.profile, ordinal, recorded) {
			return fillerstructurewindow.StitchResult{}, fmt.Errorf("structure window %d drifted authority", ordinal)
		}
		if !found {
			if err := r.evidence.PutStructureWindowAssessmentEvidence(ctx, recorded); err != nil {
				return fillerstructurewindow.StitchResult{}, fmt.Errorf("persist structure window %d: %w", ordinal, err)
			}
		}
		replayed, err := r.evidence.GetStructureWindowAssessmentEvidence(ctx, prepared.Authority, recorded.Record.SHA256)
		if err != nil {
			return fillerstructurewindow.StitchResult{}, fmt.Errorf("replay structure window %d evidence: %w", ordinal, err)
		}
		if !reflect.DeepEqual(replayed, recorded) {
			return fillerstructurewindow.StitchResult{}, fmt.Errorf("replay structure window %d evidence drifted", ordinal)
		}
	...
	if err != nil {
		return fillerstructurewindow.StitchResult{}, fmt.Errorf("stitch structure windows: %w", err)
	}
	if err := r.evidence.PutStructureWindowStitch(ctx, stitched); err != nil {
		return fillerstructurewindow.StitchResult{}, fmt.Errorf("persist structure window stitch: %w", err)
	}
	return stitched, nil
}
