...
func AssessSourceStructure(input SourceStructureInput) (SourceStructureAssessment, error) {
	...
	plan, err := buildStructurePlan(input.Source.DurationMs, boundaries, input.RoleClaims, input.DiscardClaims, observations)
	if err != nil {
		return SourceStructureAssessment{}, err
	}
...
func ValidateSourceStructureAssessment(assessment SourceStructureAssessment) error {
	...
	plan, err := buildStructurePlan(assessment.DurationMs, boundaries, claims, discards, normalized)
