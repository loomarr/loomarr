...
func (s *Server) registerDiagnostics(api huma.API) {
	...
	op := huma.Operation{
		...
		Description: "Returns one bounded diagnostic timeline page in newest-first or oldest-first order as typed JSON or the same records as NDJSON.",
		Tags:        []string{"diagnostics"},
...
func (s *Server) diagnosticEventsHandler(
	...
	page, err := s.diagnosticEvents.Query(r.Context(), diagnostics.EventQuery{
		From: input.From, To: input.To, Limit: input.Limit, Cursor: input.Cursor, Order: input.Order,
