...
// Incoming is a progress handoff, not a projection of every durable pipeline or decision row.
// This HTTP case pins the three facts that are easiest for separate queries to blur together:
// machine-owned work is Preparing, a Ready clip is recent success, and an old audit-only review
// is not work the household owes.
func TestFillerIncoming_SeparatesMachineWorkAndReadyClipsWithoutInventingHumanWork(t *testing.T) {
	...
	if body.RecentlyReady.Total != 1 || len(body.RecentlyReady.Rows) != 1 ||
		...
		t.Fatalf("recently ready = %+v, want the one Ready clip", body.RecentlyReady)
	}
	if body.NeedsHelp.Total != 0 || len(body.NeedsHelp.Rows) != 0 {
		t.Fatalf("needs help = %+v; an audit-only review must not become work for the household", body.NeedsHelp)
	}
}
...
func TestFillerIncoming_PagesPreparingWithTheSamePredicateAsItsTotal(t *testing.T) {
	...
	read := func(path string) calmIncomingBody {
		t.Helper()
		res := sourceReq(t, http.MethodGet, srv.URL+path, "", adminToken)
		if res.StatusCode != http.StatusOK {
	...
	second := read("/v1/filler/incoming?preparingCursor=" + url.QueryEscape(first.Preparing.NextCursor))
...
func TestFillerIncoming_RejectsAnInvalidCursor(t *testing.T) {
	srv, _, _ := newFillerServer(t)
	res := sourceReq(t, http.MethodGet, srv.URL+"/v1/filler/incoming?preparingCursor=not-a-cursor", "", adminToken)
	defer res.Body.Close()
...
func TestFillerIncoming_ProjectsOnlyAReadySplitAsDurableHelp(t *testing.T) {
	srv, st, _ := newFillerServer(t)
	putClip(t, st, filler.Clip{
		Hash: "compilation", Path: "compilation.mp4", Name: "Saturday morning commercial reel",
		Held: true, IsComposite: true,
	})
	if err := st.UpsertSplitProposal(t.Context(), filler.SplitProposal{
		ID: "split-one", ClipHash: "compilation", CreatedAt: time.Now().UTC(),
	...
	if body.NeedsHelp.Total != 1 || len(body.NeedsHelp.Rows) != 1 {
		t.Fatalf("needs help = %+v, want one current split choice", body.NeedsHelp)
	}
	got := body.NeedsHelp.Rows[0]
	if got.ID != "split-one" || got.Kind != "split_boundary" ||
		got.Question != "Where should this compilation be split?" || got.ActionLabel != "Review clips" ||
		got.ActionHref != "/filler/splits/split-one" {
		t.Fatalf("task = %+v, want the server-authored split review destination", got)
	}
}

func TestFillerIncoming_PagesNeedsHelpNewestFirst(t *testing.T) {
	srv, st, _ := newFillerServer(t)
	now := time.Now().UTC().Truncate(time.Second)
	for i := range 22 {
	...
	if first.NeedsHelp.Rows[0].ID != "split-21" {
		t.Fatalf("first Needs-help row = %q, want newest task", first.NeedsHelp.Rows[0].ID)
	}
	second := read("/v1/filler/incoming?needsHelpCursor=" + url.QueryEscape(first.NeedsHelp.NextCursor))
