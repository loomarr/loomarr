...
- Pick the search mode from the intent. If the first call comes back empty, call the tool again using the alternate mode:
  ...
  - NAMED PROGRAMMING BLOCK, LINEUP, OR COLLECTION → call catalog_search with mode "collection", media_type, and 1-8 exact constituent titles (use {"name":"Title","year":1990} for reused names). Network, genre, era, and adjacent titles never prove membership and must not pad this result.
