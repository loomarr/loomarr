...
func TestCompareTemporalStructureAssessmentsScoresConstructionTruthAndBoundaries(t *testing.T) {
	...
	for _, summary := range report.AssessorSummaries {
		if summary.ExactUnitCorrect != 3 || summary.StandaloneClassCorrect != 3 || summary.RoleComparable != 1 || summary.RoleCorrect != 1 || summary.ExactLabelCorrect != 3 || summary.Boundary.TruthTargets != 3 || summary.Boundary.ComparableTargets != 3 || summary.Boundary.Within2000MS != 3 || summary.Boundary.Within5000MS != 3 || summary.Boundary.MedianDistanceMS == nil || *summary.Boundary.MedianDistanceMS != 100 {
			t.Fatalf("assessor summary = %+v", summary)
		}
	}
	pair := report.PairSummaries[0]
	if pair.OperationallyComparable != 3 || pair.ExactUnitAgreement != 3 || pair.StandaloneClassAgreement != 3 || pair.RoleComparable != 1 || pair.RoleAgreement != 1 || pair.ExactLabelAgreement != 3 {
...
func TestCompareTemporalStructureAssessmentsSeparatesErrorsFailuresAndAgreement(t *testing.T) {
	...
	if secondSummary.AssessorID != "assessor-b" || secondSummary.OperationalFailures != 1 || secondSummary.UnitComparable != 2 || secondSummary.ExactUnitCorrect != 1 || secondSummary.StandaloneClassCorrect != 1 || secondSummary.RoleComparable != 1 || secondSummary.RoleCorrect != 0 || secondSummary.ExactLabelCorrect != 0 || secondSummary.Boundary.TruthTargets != 3 || secondSummary.Boundary.ComparableTargets != 0 || secondSummary.Boundary.MedianDistanceMS != nil {
		t.Fatalf("second assessor summary = %+v", secondSummary)
	}
	pair := report.PairSummaries[0]
	if pair.OperationallyComparable != 2 || pair.ExactUnitAgreement != 1 || pair.StandaloneClassAgreement != 1 || pair.RoleComparable != 1 || pair.RoleAgreement != 0 || pair.ExactLabelAgreement != 0 {
		t.Fatalf("pair summary = %+v", pair)
	}
	...
	for _, want := range []string{"operational_failure", "unit_error", "role_error", "model_unit_disagreement", "model_role_disagreement"} {
