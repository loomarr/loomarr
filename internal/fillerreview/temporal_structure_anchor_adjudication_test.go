...
func TestPublishTemporalStructureAnchorAdjudicationBurnsChallengeWithoutRepairingScore(t *testing.T) {
	...
	if item.Alias != fixture.target.Alias || item.Coverage != TemporalStructureAnchorReviewComplete || item.Disposition != TemporalStructureAnchorConfirmed || !reflect.DeepEqual(item.Original, item.Adjudicated) || item.EvidenceAlias == "" || item.CaseID == "" || item.SourceID == "" || item.FamilyID == "" || !reviewSHA256(item.SourceSHA256) {
...
func newTemporalStructureAnchorAdjudicationFixture(t *testing.T) temporalStructureAnchorAdjudicationFixture {
	...
	for index := range second.Assessments {
		if second.Assessments[index].Alias == target.Alias {
			...
			second.Assessments[index].Segments[0].Role = fillereval.TemporalSegmentRole(second.Assessments[index].Role.Kind)
		}
	}
...
func temporalStructureAnchorAssessmentSet(
	...
	for _, truth := range challenge.Cases {
		duration := durations[truth.Alias]
		segments := temporalStructureTruthSegments(truth, duration)
		claims := make([]TemporalStructureSegmentClaim, 0, len(segments))
		for _, segment := range segments {
			claims = append(claims, TemporalStructureSegmentClaim{
				StartMS: segment.StartMS, EndMS: segment.EndMS, Role: segment.Role,
				DecisiveAtMS: []int64{segment.StartMS + (segment.EndMS-segment.StartMS)/2},
				Reason:       "constructed full timeline",
			})
		}
		decisive := []int64{claims[0].DecisiveAtMS[0]}
		switch truth.Unit {
