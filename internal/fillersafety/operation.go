...
func validHostedIdentity(identity hostedCallIdentity) bool {
	return boundedLedgerID(identity.RequestedProvider) && boundedLedgerID(identity.RequestedModel) &&
		boundedLedgerID(identity.ResolvedProvider) && boundedLedgerID(identity.ResolvedModel) &&
		boundedLedgerID(identity.UpstreamProvider) && validSHA256(identity.CapabilitySHA256) &&
