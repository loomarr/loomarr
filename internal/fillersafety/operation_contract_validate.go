...
func ValidateHostedCallReservation(command HostedCallReservation) error {
	if !boundedLedgerID(command.EventID) || !boundedLedgerID(command.RunID) ||
		...
		!boundedLedgerID(command.RequestedProvider) || !boundedLedgerID(command.RequestedModel) ||
		!boundedLedgerID(command.UpstreamProvider) || command.Ordinal < 0 || command.CreatedAt.IsZero() ||
...
func ValidateHostedCallSettlement(command HostedCallSettlement) error {
	...
	if command.Failure == FailureNone {
		if !validSHA256(command.ResponseSHA256) || !boundedLedgerID(command.ResolvedProvider) ||
			!boundedLedgerID(command.ResolvedModel) || !boundedLedgerID(command.UpstreamProvider) ||
	...
	if !validSettlementFailure(command.Failure) || command.Failure == FailureInterrupted || command.Failure == FailureBudget ||
		...
		!optionalLedgerID(command.ResolvedProvider) || !optionalLedgerID(command.ResolvedModel) ||
		!optionalLedgerID(command.UpstreamProvider) || !optionalLedgerID(command.GenerationID) {
		return ErrEvaluationInvalid
	}
