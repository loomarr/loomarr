...
func TestCompleteAudioWindowProposerCoversEverySourceMillisecondExactlyOnce(t *testing.T) {
	...
	for _, durationMS := range []int64{1, completeAudioWindowMS, completeAudioWindowMS + 1, completeAudioWindowMS*3 + 127} {
		t.Run(durationTestName(durationMS), func(t *testing.T) {
			...
			output, err := proposer.Propose(context.Background(), request)
...
func TestCompleteAudioWindowProposerRejectsRatherThanTruncates(t *testing.T) {
	...
	if output, err := proposer.Propose(context.Background(), maximum); err != nil || !output.Complete || len(output.Candidates) != maxProposedCandidates || output.Candidates[len(output.Candidates)-1].EndMS != maxCompleteAudioWindowMS {
	...
	if output, err := proposer.Propose(context.Background(), request); !errors.Is(err, ErrEvaluationInvalid) || output.Complete || len(output.Candidates) != 0 {
		t.Fatalf("output=%+v err=%v", output, err)
	}
	cancelled, cancel := context.WithCancel(context.Background())
	cancel()
	request.DurationMS = completeAudioWindowMS
	if _, err := proposer.Propose(cancelled, request); !errors.Is(err, ErrEvaluationInvalid) {
