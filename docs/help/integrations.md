...
OpenRouter can run every filler AI capability with that same key. Under Settings → AI, choose
**Connected speech service** and leave its address blank to reuse OpenRouter, or open Advanced to
give speech recognition its own OpenAI-compatible address, model, and key. The chat, vision, and
speech-to-text model ids remain separate because they accept different inputs.
...
keyframes, and clip transcription sends audio in sub-minute chunks. Choosing a connected speech
service also sends a short audio sample when Loomarr checks a clip's language.
