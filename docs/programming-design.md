...
- **Transparency at review:** the proposal shows the policy's effect — so gaps are visible *before* approval, and the fix (rate your media, or relax the policy) is a human decision.
  ...
  - ⚠ **Ordering is load-bearing.** The refusal runs *after* `groundPolicy`, because `ceilingAdmittingPicks` reads the picks to decide the ceiling while the refusal reads the ceiling to decide the picks. Reversed, a pick would be refused by a ceiling that the pick itself was about to lift.
  - ⚠ **An UNRATED pick is deliberately NOT refused**, even though the §4 gate fails closed on unrated under a kids ceiling. At proposal time "unrated" overwhelmingly means *not looked up yet*: the reconcile heal (§389) exists to fill an empty rating once the title is in the library, and a library cannot rate an acquisition it does not have — TMDB enrichment for those is best-effort. Refusing here would reject titles that go on to air fine, and nothing is *admitted* by the leniency because the enforcer still fails closed at airtime. Only a certain refusal is worth stating.
...
- The LLM's job is to *infer* a ceiling from a young-audience intent ("cartoons," "Saturday morning," "for the kids" → `TV-Y7`) or retain the canonical boundary the operator named ("keep it PG-13" → `PG-13`). The value is shown as an editable chip in review, and enforcement is the ladder comparison, nothing else.
- **The ceiling is an explicit operator limit or a kids/teen guardrail, not a general default.** An unqualified channel is an **adult-default** channel — "1980s Action Heroes" obviously includes its R-rated films (Die Hard, Predator, The Terminator). So **a proposed ceiling is kept only when the intent carries a kids/teen signal or explicitly names a canonical TV/film rating limit**. With neither signal, any model-proposed ceiling is dropped (→ no ceiling, everything admitted) — a small model's reflexive "action might be violent, better cap it at TV-14" must not silently strip the R-rated content the channel is *about*. This is enforced deterministically in `groundPolicy` (the prompt says "adult/no mention → omit," but the model isn't trusted to obey it). Recognition of short film tokens (`G`, `PG`, `R`) requires rating/limit context so ordinary prose cannot accidentally pin a ceiling. **The safety asymmetry is absolute:** dropping an unjustified ceiling only ever loosens; a retained ceiling is enforced fail-closed and a known over-ceiling pick is refused at review.
- **Only an inferred kids ceiling may auto-raise to admit picks.** If that inferred ceiling is stricter than the channel's own grounded kids-band picks, it is raised just enough to avoid silently emptying the channel, and never above `TV-PG` (`KidsCeilingRank`). An **explicit rating limit is never raised**: `PG-13`/`TV-14` remains at that ladder rank even when the model also picked `R`/`TV-MA`; those harder picks are refused as `over_ceiling` instead of weakening the operator's boundary.
