...
- **One bind path (no drift).** Materializing an approved proposal onto a channel — refresh `ch.Lineup` + `ch.Policy` from the newest approved proposal, then apply the §2.1 ownership rules, reconcile — is extracted behind a **`ChannelBinder`** the composition root wires once. The policy refresh is **not** a wholesale overwrite: it goes through the single merge that (a) skips any proposal-owned field the operator has marked **operator-set** (§2.1 stickiness), (b) keeps `filler`/`window`/`autoCurate` always, and (c) merges `rules` by **provenance** (replace `llm`, keep `operator`). The manual-approve handler AND every auto-approve path (the existing per-user grant *and* auto-curate) call it, so "what approving does to a channel" has a single implementation. (This also closes a latent gap: before the extraction, a *per-user* auto-approved refine enqueued acquisitions but never rebound its channel.)
