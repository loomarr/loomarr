# PROGRESS journal — archived narrative
...
## 2026-09-08 — prepared handoff investigation through `324450f5`

This is the former Active work entry, preserved before recording the integrated continuation repair.
Its scratch-only and unresolved-production statements describe that earlier checkpoint.
Current status belongs in `PROGRESS.md`; this record does not supersede it.

| Workstream | State at checkpoint | Branch | Investigation and evidence |
| --- | --- | --- | --- |
| Production-path playout load certification | cancellation delivered; integrated harness gate and composition review passed; full qualification pending | `playout-load-cert-1037` ([#1037](https://github.com/loomarr/loomarr/issues/1037), delivery [#1116](https://github.com/loomarr/loomarr/issues/1116)) | Cancellation diagnostics #1149 is accepted at `4719e1de`. Local shutdown `e361aa77`, input bounds `b5e7ab56`, and HTTP/HLS readers `5e8a8e78` retain their recorded race, lint and isolated real-FFmpeg evidence. Publication checkpoint `14c6e6f1` passes `go test -race ./cmd/playout-load-cert`, `go test -race ./internal/playoutcert ./internal/testkit/playoutcertfixture`, `go test -race -tags ffmpeg -run ^TestSynthetic ./internal/playoutcert`, the tagged command-entrypoint certification test, and pinned lint. Exact immutable output, cleanup downgrade, private-source audit and artifact preservation are reviewed. The canonical gate exposed a synthetic shutdown-registration race and an inbound API dependency; repair `7e02fbba` moves target composition to the app layer, closes registration admission before shutdown waits and preserves all nine FFmpeg scenario assertions. Independent reviews passed, current-main integration is complete, and `make verify BASE=66235e2439e6f8f1c2a82d96e32be92cac069e02` passed at `654360f4` after sanctioned architecture-map regeneration. The integrated host-FFmpeg real-media suite passed (257.155s). After distinct programme fixtures `e2e99679`, the isolated Linux ARM64 run using the production-pinned FFmpeg exited 1. All three failing scenarios share the prepared raw-witness boundary failure; a two-channel reproduction measures progress ending before the required late interval, while the transcode control passes. A parent discontinuity-threshold candidate passed those three cases but was rejected when matched packet evidence showed second-programme audio shifted about 271 ms early; the production option is restored. A packet-matched A/V synchronization regression passes the default mux and rejects that candidate on production-pinned FFmpeg; A replay of the captured, actually forwarded child prefix with stdin held open reproduces a 3.24-second output gap before EOF after shared audio/video discontinuity corrections; the production repair remains open. Packet hashes confirm that buffered predecessor video is shifted by the successor audio correction. Explicit video PES lengths still fail the original case; bypassing parsers with initial non-keyframe copying preserves payloads and removes the large gap but introduces a 42.67 ms backward audio timestamp, so that variant is rejected too. Dedicated decoded A/V checkpoint `51c49cdc` passed focused host race and pinned Linux ARM64 tests; independent review found a malformed-line acceptance defect, now repaired with regressions that fail against the old parser and pass against the fix. HLS integration remains required. Checkpoint `e25070c6` passes `make verify BASE=45574bdba9e3be2b8ebc1810509602e06ec3330a` (Go, full Go, docs and policy). Published `e7325cfd` passes affected CI ([34157233638](https://github.com/loomarr/loomarr/actions/runs/34157233638)); draft [#1157](https://github.com/loomarr/loomarr/pull/1157) remains incomplete because repository checks do not qualify the failing production-pinned continuity scenarios. A no-B-frame synthetic preparation plus shared child timeline and immediate parent output flushing preserves all packet payloads and one common A/V timestamp shift at zero and 8 ms source offsets. A targeted 37 ms offset produces a 27 ms video overlap; the parent repairs one video timestamp independently, disproving universal cut correctness despite continuous output. These are bounded scratch experiments, not a production repair; source-cut handling remains required. A separate prepared-child failure repair now waits for the natural exit before treating stdout EOF as completion: a real child emitting a prefix then exiting 7 previously hid the failure and made the pump wait until scheduled end. The regression fails before the fix and passes with race detection afterward (2.273s), including successful exit, cancellation and explicit close. The corresponding live HTTP regression also exposed a child exiting 7 after a prefix being reported as a clean response. The live handler now waits for natural exit before completion and aborts a failed partial response; the real HTTP success/failure, repeated-request and disconnect tests pass with race detection (4.225s). These recovery prerequisites do not resolve the handoff timestamp failures. The certification live resolver also omitted its elapsed source offset, making its published end coordinate too early; a cross-resolver playhead/offset/end regression rejects the omission and now passes with race detection. Corrected live-source timing must be included in fresh certification evidence; the combined continuation/timeline/no-reordering candidate remains scratch-only. Ordinary live-transcode HLS programme identity/media correspondence, remaining fault/codec/copy cohorts, final integrated review and canonical gates remain required. No hardware, operator-media or live-installation certification is claimed. The raw-viewer byte-bound repair replaces the eight-fragment mailbox with a cancellable 512 KiB FIFO; the original implementation loses small-fragment bursts (8/1,024 and 3,008/48,128 bytes), while the new regression passes. The full playout package passes race detection (11.019s), including stalled-reader isolation; raw HTTP startup/disconnect checks pass (8.006s). With the byte-bound repair and the unchanged scratch continuation overlay, all three original pinned scenarios passed twice (six passes, no initial-media failures). Queue checkpoint `a6da65d8` passes `make verify BASE=fe46651a28f08c6fa575fa50a920bd12576eb027` (Go, full Go, docs and policy). The first full gate hit an existing command-report test's 90-second deadline; it passed unchanged in isolation (51.633s) and the full retry passed. These candidate results do not certify the production continuation contract, which remains scratch-only. The continuation eligibility regression now covers late predecessor completion, an on-time predecessor followed by delayed source resolution, gaps, missed Airings and empty output. Production preserves the five-second current offset in all five cases (race PASS, 1.267s); the scratch candidate fails both delayed adjacent cases, and an added predecessor-completion deadline still fails delayed lookup. Standard-library fake time keeps the five-second delay deterministic and fast. Regression checkpoint `8651fb1d` passes `make verify BASE=31989eb9cbce26fc9db49ec35b3546909a59ba35` (Go, docs and policy). An isolated pinned-parent replay retains all 258 video/484 audio packets with one shared timestamp shift but delivers the delayed next programme about five seconds late, so timestamp preservation alone cannot qualify live continuation. |


### Playout certification checkpoint before HLS timestamp repair (2026-09-08)

| Workstream | State | Branch | Plan / evidence |
| --- | --- | --- | --- |
| Production-path playout load certification | continuation integrated; pinned scenarios and local gates pass; final qualification pending | `playout-load-cert-1037` ([#1037](https://github.com/loomarr/loomarr/issues/1037), delivery [#1116](https://github.com/loomarr/loomarr/issues/1116), draft [#1157](https://github.com/loomarr/loomarr/pull/1157)) | Application checkpoint `6f8f38c5` integrates timed prospective prepared opening, separate startup/lifetime cancellation, a shared prepared/live/card clock, copied-seek preroll rejection and packaging-v2 output verification. It retains child-exit recovery and the byte-bounded viewer queue. All nine maintained synthetic scenarios pass on pinned Linux ARM64 FFmpeg (243.347s), including the original prepared-boundary failures; real HTTP chain/fallback checks pass (8.837s), and packet/seek/copy/trimmed-media checks pass (19.539s, including H264/HEVC across all four copy plans). Actual packaging and reordered-output rejection pass (0.745s). Test-cleanup follow-up `41755aa4` passes `make verify BASE=324450f5658d27af1f9f02c5b90c5789385df2dd` (Go, full Go, docs, policy). The initial full gate hit an existing 90-second command-report fixture deadline; the unchanged test passed in isolation (47.543s), then the full retry passed (command package 103.710s). That timeout sensitivity remains unresolved. Signal flushing repair `78533235` passes `make verify BASE=5264a2c6baea09937b2018131baf7038c15c27e9` (Go, docs, policy): a pinned held-open regression fails with zero video observations before EOF on the old code and passes in 0.14s with direct metadata flushing; focused race checks pass (1.477s). Scratch ordinary-HLS evidence found an unresolved 217–227 ms audio phase lead at two transitions; an isolated eight-case child seek probe did not reproduce it. Ordinary-HLS composition `96797218` passes the prepared-miss → signed TS/A/V → shutdown/disposal regression under the host race detector (9.40s), plus `make verify BASE=12bc08fee3e4cd3d4b69ea3c900c979840336fa5` (Go, full Go, docs, policy). Pinned public-route probes pass delivery/progress at two and seven transitions (16.29s/44.35s); the longer observation still finds eight inconsistent audio windows and approximately 0.2-second boundary gaps, so A/V qualification remains open. Both prepared and live HLS need the maintained private signature correspondence and late-progress consumer; mixed prepared/live transitions, remaining qualification cohorts and final independent review also remain open. No operator, hardware, provider or wider-beta qualification is claimed. Earlier investigation evidence is preserved in the [progress journal](progress-journal.md). |
...
### 2026-09-08 — Full affected gate exposes the live audio publication blocker

Accumulated source was committed as `dbaaa306415ff2f1df92240563f236c3c09e2662`. `make verify BASE=4d1e9edfe4e76783c9cab32497f954c1f0ab60ef` selected local Go/full-Go/docs/policy evidence and exited **2** at the command package. The affected closure lint passed; API (**103.220s**), app (**111.191s**), certification (**74.416s**) and shared fixture (**1.153s**) race suites passed. Later verification steps are not claimed as complete.

Both positive actual-run publication tests failed: `TestCleanupFailureIsPersistedAsUncertifiedReport` reached its existing 90-second deadline, and `TestPublishReportUsesActualRunPublication` completed with copy, programme-boundary and overload failures. Their shared fixture still supplied 100 prepared and five transcode Channels but lacked the newly required copy Channel. It now also supplies the separate real generated-copy Channel. The pre-publication authority and zero-failure assertions remain intact; diagnostics distinguish a premature verdict from failed workload evidence instead of printing a misleading forged-verdict error for both.
