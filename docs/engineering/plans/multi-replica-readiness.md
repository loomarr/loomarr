...
| Suggestion jobs | A claims then dies; B reclaims and produces one terminal result. | PR #453 added versioned Attempts, expired-lease recovery, and stale-attempt compare-and-swap with shared SQLite/Postgres conformance. The required two-process Postgres kill/reclaim proof remains unrecorded, so this row is implemented but not yet cluster-certified. |
...
| Approval quota | One proposal commits once; two proposals competing for one remaining unattended slot approve at most one. | Every approval for a requester is ordered. PostgreSQL binds its transaction-scoped advisory lock, automatic quota reads, and approval commit to one transaction/session; manual approval participates without quota rejection, and post-commit work runs after ordering is released. Store conformance covers the one-slot race and canceled-waiter reacquisition on SQLite/Postgres, with a Postgres two-independent-pool contention proof; the full two-process promotion suite remains required. |
