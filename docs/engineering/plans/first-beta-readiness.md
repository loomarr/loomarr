...
| The product calls AI/TMDB optional while the only normal channel-origination UI requires suggestions. | `README.md`; `docs/install/index.md`; `docs/help/quickstart.md`; `internal/api/proposals.go`; `docs/design.md` §12 | Unowned product/documentation decision. For beta, state the defining-feature prerequisite explicitly unless a non-AI UI door is added. |
| `--profile postgres` starts Postgres but leaves Loomarr on SQLite. | `docker/compose.yaml`; `docs/install/docker.md`; `docs/help/quickstart.md` | Unowned release plumbing. Provide a wired override/profile path and a Compose contract test, or drop Postgres from beta. |
| Install/upgrade Compose points at `loomarr:latest`, while releases publish GHCR. | `docker/compose.yaml`; `docs/install/upgrading.md`; `.github/workflows/release.yml` | Unowned release plumbing. Parameterize the GHCR semver image and document pin/upgrade/rollback. |
| The release image is not rebuilt in CI for all Docker build inputs; tag publishing does not require or run the full gate. | `.github/workflows/ci.yml`; `Dockerfile`; `.github/workflows/release.yml` | Unowned release pipeline. Expand build-input coverage and add a tested release-candidate gate/tag policy. |
| Distributed binary/model notices and integrity evidence are incomplete. | `Dockerfile`; `THIRD_PARTY_NOTICES.md` | Unowned. Inventory Whisper and model artifacts, verify executable downloads, and align SBOM/license wording before publishing. |
| Backup prose overstates scope and restore guidance is not a drill. | `.env.example`; `docs/install/{docker,upgrading}.md`; `docs/design.md` durability section | Unowned. Correct the scope and prove SQLite and Postgres restore procedures. |
...
4. Runtime proof: merge browser/controller and decoded-playback certification; run restore drills and release-candidate image smoke on both architectures.
