...
| Layout, surfaces, typography, artwork, focus, and primitive actions | `design-system` primitives | `Screen`, `Surface`, `Text`, `ArtworkFrame`, `FocusSurface`, `Action`, `Badge`, `Field`, and `ProgressTrack` | partial | Add safe-area/overscan-aware layout, responsive disclosure, scroll/stack seams, and complete disabled/pressed/selected/error/focus contracts |
