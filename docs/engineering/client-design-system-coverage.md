...
| Guide product rules | `core/guide`; `ui` `GuideSurface`, `GuideExperience`, and `GuideJourney` | Shared geometry, metadata formatting, selection, movement, latest-request-wins fetch, empty/error recovery, and authoritative generated `/v1/guide` window now feed the production mobile/TV Guide; tuning its selection enters the still-mounted player without inventing metadata. Native artwork/logo renderers consume the generated image paths through the paired origin-locked adapter | partial | Connect virtualization and the TV navigation adapter, then prove tune/back/focus restoration, populated artwork, and refresh with the real API journey |
