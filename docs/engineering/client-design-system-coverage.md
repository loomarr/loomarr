...
| Guide TV mechanics | `ui-tv` Guide adapter | TV-only package now owns deterministic grid/filter D-pad movement, disabled-filter skipping, time-anchor retention, tune/filter activation intent, focus restoration after catalog change, and a bounded 100-channel row window with explicit position labels | partial | Connect the controller to native focus refs and rendered row window, then prove remote repeat, overscan, focus restoration, and 1080p/4K viewport behavior in emulator and physical Shield |
