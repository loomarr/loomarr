...
| Application navigation shell | `ui` `ClientNavigation` and `ClientShell` plus host Back adapters | Shared Watching/Guide/Surf destination intent, labelled selected-button navigation, icon vocabulary, keyboard reachability, TV preferred focus, and the Guide/Surf → Watching → platform-home Back rule now run through production native surfaces over the still-mounted player; confirmed disconnect remains in the shell | partial | Prove route intent, focus restoration, surrounding-control handoff, and final Back exit on browser, touch device, and TV hardware |
| Guide product rules | `core/guide`; `ui` `GuideSurface`, `GuideExperience`, and `GuideJourney` | Shared geometry, metadata formatting, selection, movement, latest-request-wins fetch, empty/error recovery, and authoritative generated `/v1/guide` window now feed the production mobile/TV Guide; tuning its selection enters the still-mounted player without inventing metadata | partial | Connect virtualization and the TV navigation adapter, then prove tune/back/focus restoration, artwork, and refresh with the real API journey |
| Guide TV mechanics | `ui-tv` Guide adapter | TV-only package now owns deterministic grid/filter D-pad movement, disabled-filter skipping, time-anchor retention, tune/filter activation intent, focus restoration after catalog change, and a bounded 100-channel row window with explicit position labels; a Storybook remote workshop proves the same controller through Arrow/Enter interaction in both themes | partial | Connect the controller to native focus refs and rendered row window, then prove remote repeat, overscan, focus restoration, and 1080p/4K viewport behavior in emulator and physical Shield |
...
| Player state and transport | `player` root interface plus web/native adapters | The dependency-stacked P4 branch adds a generated-contract-derived root interface, latest-request-wins controller, intent-sensitive initial tuning, exact/wrapped tuning, identity reconciliation, bounded previous/recent history, Loomarr-owned overlay visibility, signed-URL source, origin-locked paired transport, and one Expo Video native adapter with serialized replacement and synchronous pause/release; 17 interface/adapter tests, Expo Doctor, and Android/iOS/TV bundles are green | partial | Add the browser adapter, live-time-shift state, diagnostics, catalog SSE reconciliation, and real first-frame/background/recovery evidence before P4 completion |
| Player chrome and timeline | `ui` player presentation consuming `player`; `ui-tv` number-entry adapter | Shared `WatchingSurface` keeps one supplied player mounted behind Channel identity, tuning/error/dead-air feedback, bounded transient chrome, pause/Play/Go Live, and explicit previous/channel/Guide/Surf/retry actions across touch/TV native stories; the TV adapter retains the exact three-digit/1.2-second number-entry contract and interface tests cover the observable states | partial | Add authoritative now/next timeline/progress and prove touch/keyboard/remote traversal at every required viewport |
| Surf rail | `ui` `SurfRail` and `SurfJourney`; `ui-tv` Surf adapter | The production mobile/TV rail now preserves the still-mounted player, filters the authoritative Guide against the server-declared playable catalog, keeps empty Favourites honest, derives Recent/All with now/next/progress, restores selection by identity, and tunes through the shared player; confirmed disconnect and Back remain available in the surrounding shell | partial | Bind the explicit TV navigation controller to native focus refs, add artwork renderers and server version identity, then prove catalog refresh, tune, previous-channel, and viewport behavior on browser, touch device, emulator, and Shield |
...
| TV mechanics | `ui-tv` root interfaces | Pairing/shell remote traversal and tested exact number-entry buffering exist; Guide/Surf focus logic remains disconnected from the production native surfaces | partial | Physical 4K Shield plus 1080p host evidence for D-pad, OK, Back, number keys, focus restoration, overscan, and ten-minute traversal |
