...
| Layout, surfaces, typography, artwork, focus, and primitive actions | `design-system` primitives | `Screen`, `Surface`, `Text`, `ArtworkFrame`, `FocusSurface`, `Action`, `Badge`, `Field`, and `ProgressTrack`; P3.5 now supplies provider-owned platform insets, a 48-unit TV overscan policy, and an edge-to-edge viewport frame | partial | Add responsive disclosure and scroll/stack seams, then complete disabled/pressed/selected/error/focus contracts |
| Form and selection controls | `design-system` primitives; web semantics adapter where required | Shared `Field` and `Action`; checkbox, select, switch, menu, tabs, tooltip, and dialog remain legacy-web implementations | partial | Shared visual/state contracts cover known mobile and administrative needs; DOM-only semantics remain narrow adapters |
| Feedback and recovery | `ui` feedback module | Empty, error, placeholder, and status treatments exist only in the legacy web tree; P3.5 implementation is in progress | claimed | Shared loading/empty/error/offline/retry/permission states with useful accessible copy and density variants |
| Channel and programme identity | `ui` identity and metadata modules | Shared `ProgrammeCard`; channel identity and complete episode/airing metadata are still app-local | partial | One shared title, channel number/logo, series/season/episode, time, description, badge, artwork, and fallback contract |
...
| Overlay and modal composition | `ui` overlay module plus platform portal adapter | Dialog/menu implementations are legacy web; no shared TV/touch overlay contract | missing | Scrim, stacking, focus trap/return, dismissal, safe area, reduced motion, and non-blocking playback composition proven |
