...
| Layout, surfaces, typography, artwork, focus, and primitive actions | `design-system` primitives | `Screen`, `Surface`, `Text`, `ArtworkFrame`, `FocusSurface`, `Action`, `Badge`, `Field`, and `ProgressTrack`; P3.5 now supplies provider-owned platform insets, a 48-unit TV overscan policy, an edge-to-edge viewport frame, and semantic action states | partial | Add responsive disclosure and scroll/stack seams; preserve the proven disabled/pressed/selected/error/focus control states |
