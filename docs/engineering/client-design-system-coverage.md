...
| Form and selection controls | `design-system` interaction and selection modules; web semantics adapter where required | Shared `Action`, `Field`, `Toggle`, `ChoiceGroup`, `Tabs`, `MenuList`, `SelectControl`, and `Hint` own their cross-platform presentation and state contracts with pointer/touch/TV stories; browser interaction coverage proves selection, disabled, expanded, and menu behavior; dialogs use the shared `ModalOverlay` | partial | Add platform adapters for tab arrow traversal, select/menu trigger placement, and tooltip hover/long-press, then prove native input behavior rather than duplicating product rules |
