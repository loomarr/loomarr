...
- **Step → group mapping:** claim (auth, pre-settings) → Connections/media-server → Connections/Tunarr (+ media-source check; **saving auto-wires Live TV into the guide** — no separate step) → Connections/requester + **webhook handshake** (displays the generated secret's URL, listens for `Test`) → AI (skippable; includes the §8.1 model picker) → Filler (skippable; drop-folder path + optional starter ingest targets on `loomarr:filler`) → guided first channel.
