...
| --- | --- | --- |
| **General** | Application-wide identity and addresses. **Share invitation and recovery links** owns `access.public_url`, labelled **Recipient-facing Loomarr address**. When empty, the editor stages the current browser origin as the default; the operator saves it or changes it when recipients reach Loomarr elsewhere. The server never derives this persisted value from request headers. | — |
...
(`plain`, `duration`, `bytes`, `cron`, `path`, `language`, `switch`) beside its validation kind. Validation
...
- **Suggestions unavailable** → `POST /v1/proposals` rejects before creating a Job with a 409 problem whose stable type identifies the first actionable blocker. Missing provider/model setup is `feature_not_configured` and deep-links to Settings→AI; a configured provider and selected model with missing TMDB grounding is `grounding_not_configured` and deep-links to the TMDB block on Settings→Connections. The response never calls AI unconfigured when only grounding is absent. The Add channel panel reads this authoritative diagnosis rather than re-deriving readiness, and renders it before any retry.
  The panel preserves the complete draft Intent (description and constraints) in session storage while the operator follows a setup link. Settings carries a visible **Return to channel** action; returning reopens Add channel with that draft restored, but never auto-submits a paid model request. A successful Proposal Job, **Start over**, or an intentional panel close clears the draft. A member who cannot edit Settings receives administrator-directed guidance instead of an unusable admin link. Never a stack trace, never a dead button, never a false diagnosis.
...
The rule that survives: **one function computes both the feature verdict and its ordered missing prerequisites, and every consumer reads it.** The checklist, the tab states, API gating, and recovery copy never re-derive availability — only the *inputs* differ, never the seam.
