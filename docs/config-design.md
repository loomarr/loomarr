...
| **Notifications** | The recipient-reachable access URL plus outbound email enablement, SMTP endpoint/security/authentication, and sender identity. It explains that QR/copy Invitations need only the access URL while email additionally needs the SMTP fields. SMS and product/activity notifications are not presented as available. | **Send test email** validates the current staged values without saving or queuing a domain Notification intent; its recipient is entered in the action and is never persisted as a setting. |
