...
| **Notifications** | A readiness summary separates recipient links from optional account email. The recipient-reachable access URL owns QR/copy Invitations; a dedicated switch progressively reveals SMTP endpoint/security/authentication and sender identity only when email is enabled. SMS and product/activity notifications are not presented as available. | **Send test email** validates the currently saved values without queuing a domain Notification intent; its recipient is entered in the action and is never persisted as a setting. Unsaved edits disable the action so the test cannot imply it checked values that are not yet active. |
