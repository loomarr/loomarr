...
| `playout.quality_tier` (`PLAYOUT_QUALITY_TIER`) | enum | `balanced` | The picture-versus-channel-count target. Resolved at each program boundary against your measured capacity and current load, so quality adapts as channels come and go rather than being fixed when a channel is made. _(one of: efficient \| balanced \| quality)_ |
