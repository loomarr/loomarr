...
| `filler.language` (`FILLER_LANGUAGE`) | Filler — Clip eligibility and sound | string | `en` | The language filler is expected to be in. A clip whose speech is confidently something else is rejected; a clip with no speech at all is always kept. Empty turns the language check off. |
