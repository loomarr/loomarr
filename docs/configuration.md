...
| `playout.memory_reserve_mb` (`PLAYOUT_MEMORY_RESERVE_MB`) | Playback | int | `2048` | Host memory, in MiB, that hardware encoding must leave available. Background preparation stops starting new encodes before free memory would fall below this, and live playback reclaims memory from preparation before falling back to software. Set 0 to disable the host-memory check. Changes apply to the next encode without restart. _(advanced)_ |
| `playout.encode_memory_mb` (`PLAYOUT_ENCODE_MEMORY_MB`) | Playback | int | `0` | Host memory, in MiB, one hardware encode is expected to use. Leave at 0 to use 1024 MiB, or more if the encoder capability check measured more on this machine; set it when real encodes here use more, for example 4K or HDR tone mapping. Changes apply to the next encode without restart. _(advanced)_ |
