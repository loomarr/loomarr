...
| `llm.url` (`LLM_URL`) | AI | url | — | For Ollama, enter its address, such as http://ollama:11434. For any OpenAI-compatible service, local or hosted, enter the API base ending in /v1. Loomarr fills this in for OpenRouter; Custom stays editable. |
...
| `llm.api_key` (`LLM_API_KEY`) | AI | secret | (secret) | API key for the OpenAI-compatible AI service, if it requires one. This is separate from the speech-to-text key. Never shown again after saving. |
...
| `asr.provider` (`ASR_PROVIDER`) | AI | enum | `whisper` | The app-wide service that turns speech into timed text. Use built-in Whisper, the configured lineup AI service, or a separate OpenAI-compatible speech service. _(one of: whisper \| hosted \| openai)_ |
| `asr.model` (`ASR_MODEL`) | AI | string | `openai/whisper-large-v3` | Model used by the selected remote speech-to-text service. It must return timed transcript segments. |
| `asr.url` (`ASR_URL`) | AI | url | — | Address of the separate speech-to-text service, ending in /v1. Loomarr adds /audio/transcriptions. |
| `asr.api_key` (`ASR_API_KEY`) | AI | secret | (secret) | API key for the speech-to-text service. Loomarr never sends the lineup AI key to this service. Never shown again after saving. |
...
| `filler.transcribe.enabled` (`FILLER_TRANSCRIBE_ENABLED`) | Filler — Clip review | bool | `false` | Listen to clips whose source told us almost nothing and write down what they say, so Loomarr can work out the brand and era. Uses the app-wide speech-to-text service configured under AI. _(advanced)_ |
