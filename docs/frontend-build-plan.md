...
- **`apps/web`** — Vite + React 18 + react-router v6 + Tailwind v4 + shadcn/ui (new-york). `AppShell` + routing skeleton + the SSE provider + a hand-written typed client for the non-generated routes (`auth`, `bootstrap`, `import`, `sync`, `events`, `backup`) living in `packages/core` (shared, so mobile reuses it).
