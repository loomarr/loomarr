...
- **System probe** (`GET /v1/system/llm`, admin) returns the active provider + model and **two curated catalogs**:
  ...
  - **Hosted providers** — a curated list (OpenRouter/OpenAI/Anthropic/Groq/Gemini) each with its OpenAI-compatible base URL, a few **recommended tool-calling models** (with a one-line cost/why), and a **`keyConfigured`** flag. The API key is **never echoed back** — the probe reports only whether one is set. A hosted provider needs no hardware fit; its "readiness" is *key present (and, on demand, valid)*.

  Both catalogs are curated, in-code truth (not a live registry scrape) so recommendations are deterministic and reviewable.
