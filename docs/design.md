...
- **Settings** (admin, route `/settings`) — **seven tabs**, one per thing an operator configures: *Connections* (the media server, requester, and Tunarr, plus `/readyz`, provider config visibility, and the re-runnable **connection checklist** of §13 as the troubleshooting console) · *AI* (provider/model, including the in-app **model manager** of §8.1 — probe, catalog, hot-swap, streaming pull) · *Channels & playback* · *Filler* · *Tasks* (the **job console**: every §18.1 scheduled job, its cron, last run, and Run-now) · *Users & security* (incl. **secret regeneration**) · *Advanced*. The typed registry, `env > database > default` resolution, hot-apply, the save bar, and the secrets lifecycle are `config-design.md`'s domain — it wins on those mechanics; this row records only *where the surfaces are*.
