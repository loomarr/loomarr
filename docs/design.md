...
Store interface:
  ...
  ListProposals(status[, user])           # approval queue; approval history
...
| POST | `/v1/proposals` | Start a **proposal job** from a complete typed intent and return its `jobId`. Accepting the request always creates a fresh caller-owned execution identity; a semantic cache may reuse grounded proposal content, never another caller's job, ownership, or decision history (§8). |
| GET | `/v1/proposal-jobs/{jobId}` | Authoritative state of one proposal job: its complete intent, `queued|running|done|failed` status, attempts and timestamps, safe failure (when failed), and newest proposal in any decision state (when one exists). A member may read only their own job; an admin may read any. Cross-user reads are 403, and ownership is resolved from the session rather than a caller-supplied user id. |
| GET | `/v1/proposal-jobs?mine=true&status=queued|running|done|failed` | Bounded proposal-job history for **My requests**, including in-flight and failed executions that have no proposal. `mine=true` is session-scoped; an unscopable `API_TOKEN` caller receives an empty list rather than all users' work. Admin-wide diagnosis omits `mine`; no `?user=` escape hatch exists. |
| GET | `/v1/proposals?status=…&mine=true` | List durable proposal artifacts by decision status (`submitted` = the admin approval queue). **`mine=true` scopes the list to the caller's own proposals** and remains useful for approval history; it is not execution history because a failed job produces no proposal. Scoping is resolved server-side from the session, never from a client-supplied user id. A break-glass `API_TOKEN` caller has no user record, so `mine=true` returns empty rather than everyone's. |
| GET | `/v1/proposals/{id}` | Read one **proposal artifact** by proposal id. Approve and deny use this same id; this route is deliberately not overloaded with proposal-job state. |
...
| POST | `/v1/proposals/{id}/deny` | Deny (admin) with optional reason; proposal → `denied`, and the member sees the decision through its Proposal Job in My requests. |
...
| GET | `/v1/system/llm` | Probe the LLM host + machine and recommend a provider/model (admin, §8.1): active provider + model, the distinct `configured`, `reachable`, `toolCapability` (`verified` / `unsupported` / `unverified`), and `semanticallyCertified` readiness claims, the **local** model catalog (for `ollama`: detected VRAM/version, per-model fit + recommended default, pulled flags), and the **hosted** catalog (OpenRouter + a Custom template — base URLs, recommended models, `keyConfigured`). API keys are never returned. Read-only: it performs metadata/connectivity probes only and never spends inference tokens. |
| POST | `/v1/system/llm/select` | Set the active provider + model (admin, §8.1). Persists to the settings store and **hot-swaps** the running suggester (no restart); settings override the §15 env defaults. Local model must be pulled (409 else). Hosted: accepts an optional `apiKey`, **validates** it live before committing (401/502 on a bad key), stores it as a secret (never echoed). A model known not to support tools is rejected; one whose capability metadata is absent remains unverified and must pass the explicit verification action before selection. |
...
| POST | `/v1/system/llm/verify` | Explicitly verify one provider/model's behavioral tool calling **without** swapping it (admin, §8.1). This operator action makes one deliberately tiny inference call. Its bounded, process-local result may establish reachability and tool capability for an otherwise-unverified model; it never claims semantic certification and does not survive restart. |
...
| GET | `/v1/events` | SSE stream of state changes. Frame `event:` types: `title` (provisioning) · `channel` (lineup/health) · `suggestion` (generation progress — `searching`→`reasoning`→`scoring`→`done`/`failed`, payload `{jobId, phase}`) · `llm_pull` (model-download percent) · `filler_ingest` (clip-download progress — §10) · `filler_split` (compilation-detection progress: `running` → terminal `success` carrying `{proposalId, segments}` or `error` — §10, V34) · `activity` (a new Dashboard feed row was written — §12, V32). **Latency-only:** every frame tells a client which authoritative `GET` to refetch; it never carries the sole copy of an outcome. A dropped stream, reload, or missed terminal frame therefore changes freshness, not correctness (§8). |
...
### Proposal-job reads and retries

A **proposal job** is one execution of an Intent; a **proposal** is the grounded, reviewable artifact
that a successful execution may produce. Their states are intentionally different. Jobs are
`queued`, `running`, `done`, or `failed`; Proposals are `submitted`, `approved`, or `denied`. A
failed job has no fabricated empty Proposal, and a done job may point at an already-approved or
denied Proposal. UI code must not infer execution state by listing only submitted Proposals.

`GET /v1/proposal-jobs/{jobId}` is the recovery contract. It returns the complete typed Intent so a
reload and a retry never depend on browser memory. A terminal failure is
`{code,message}` where `code` is the bounded enum `no_grounded_titles`, `timed_out`,
`provider_unavailable`, or `generation_failed`, and `message` is safe for the requester. Raw provider
responses and internal diagnostics remain in structured logs and the private diagnostic field;
they never become response text or metric labels. SSE may trigger this read sooner and may contribute
transient phase/round copy, but it cannot substitute for the read.

Retry is a new request, not a mutation of the terminal row. It submits the same complete Intent by
default and receives a fresh caller-owned `jobId`; Edit returns to that populated Intent. This keeps
attempts auditable and prevents a retry from rewriting history. The worker commits success as one
store operation — create the Proposal and transition the job to `done` — and commits failure as a
checked transition with both the safe code and private diagnostic. A semantic cache may skip model
inference, but it copies cached content into this new job and Proposal lifecycle atomically. The
scheduled refine/re-curate path may retain a Channel's stable intent reference for binding; that does
not make a previous user's execution identity reusable.

**Authorization model:** every `/v1/*` route requires a session cookie or `Authorization: Bearer ${API_TOKEN}`; approval, destructive-channel, user-management, and filler-ingestion routes additionally require `admin` (§11) — **and so do `POST`/`DELETE /v1/titles`**, since enqueuing an acquisition directly is exactly what the approval gate exists to control (members reach acquisition only via submit→approve). Read visibility is global for household resources such as channels and titles. Caller-owned resources are the deliberate exception: a member sees only their own proposal jobs and `mine` projections, while an admin may diagnose all of them. SSE endpoints authenticate via the same cookie (EventSource sends cookies same-origin). The ops surface — `/v1/healthz`, `/v1/readyz`, `/v1/metrics`, `/v1/openapi.*`, `/v1/reference`, and their bare aliases — is unauthenticated on the LAN. ⚠ That is now a *declaration*, not an absence: each of those operations is marked `RolePublic`, and `roleForOperation` fails closed for anything unmarked, so the unauthenticated surface is one greppable list rather than a property you infer from the lack of a check.
...
The **proposal job** and the **Proposal** are separate domain objects (§7). Submitting an Intent
creates the caller-owned job immediately; the Proposal does not exist until grounded generation
succeeds. The job owns execution state, the complete Intent, attempts, failure classification, and
requester visibility. The Proposal owns the lineup/policy artifact and its approval decision. This
boundary is what makes failure without a Proposal, auto-approval before the requester returns, and
recovery after a lost event representable without client-side guessing.

The worker's durable transitions, not SSE, decide the outcome. Successful generation creates the
Proposal and marks the job `done` atomically; terminal failure records a stable safe code and private
diagnostic without creating a placeholder Proposal. The read projection joins the newest Proposal
for the job regardless of whether it is `submitted`, `approved`, or `denied`. The `suggestion` SSE
frame remains useful for searching/reasoning/scoring animation, but its terminal phase only prompts a
refetch of `GET /v1/proposal-jobs/{jobId}`. Polling that read while work is active is the correctness
backstop when SSE is unavailable.

Claiming is the `queued` → `running` transition, not a lease followed by a second best-effort write.
It increments the job's attempt counter atomically; success and failure both compare that attempt as
their execution token, so an expired `running` job can be recovered after a process loss without an
older worker overwriting the replacement. A worker claims only as many jobs as it has free execution
slots. Refine/re-curate may requeue a stable job only from a terminal state and only for the attempt
the caller observed; concurrent or active requeues lose closed rather than rewriting the whole row.
Terminal SSE is emitted only after the matching durable transition commits.

The intent-hash cache is a **computation cache, never an identity cache**. Every accepted submit gets
a new job owned by its requester. On a hit, the cached grounded content is copied into a new Proposal
and both rows complete in one transaction; no requester inherits another requester's id, visibility,
approval decision, or audit trail. Retry has the same rule. Re-curation may skip an unchanged Channel
altogether or retain its stable intent binding as described in `programming-design.md` §8.2; that
optimization does not weaken fresh-request ownership.
...
**Capability needs explicit evidence:** tool-calling support varies by runtime and model, and generic endpoints expose no uniform capability API. Rich provider metadata and Ollama's declared capabilities establish `verified` or `unsupported`; absent or failed metadata stays `unverified`, never a guessed negative. The explicit §8.1 Verify action is behavioral (send a trivial tool-call request, assert a real tool call returns) and may establish runtime tool capability for an otherwise-unverified model. The §13 wizard/status reads only project metadata and cached operational evidence — they never run inference while polling. Keep the grounding tool loop to **sequential single tool calls** (no parallel-call dependence — the least-supported corner of the dialect); the grounding pipeline already ensures a weak model degrades to "no valid proposal," never to corruption.
...
- **Curation verification is distinct from connectivity.** Configured, reachable, tool-capable, and
  semantically certified are four different claims. The probe and Test action may establish only
  the first three; a green connection must never claim that a model curates Loomarr's shipped
  templates well. Semantic certification is an explicit operator/developer action against the
  configured catalog and model, using the exact canonical typed templates from §13. It records the
  provider, model, catalog fixture/fingerprint, and results; it is never run from setup polling,
  periodic health reads, or any other path that could spend hosted tokens without a person asking.
  The API exposes tool capability as the three-state value `verified`, `unsupported`, or
  `unverified`: missing provider metadata is not evidence either way. A model known to be
  `unsupported` cannot be selected for grounded curation. An `unverified` model becomes eligible
  only after the operator explicitly invokes `POST /v1/system/llm/verify`, which sends one trivial
  tool-call request and records the result in a bounded process-local cache keyed by provider,
  base URL, and model. Restarting discards that operational evidence. This check proves only
  reachability and tool-call behavior; `semanticallyCertified` stays false unless the separate
  canonical-template evaluation produced evidence. `GET /v1/system/llm`, setup checks, and other
  periodic reads never invoke inference.
...
- **Queue** (route `/queue`) — **three tabs, per the v2 mock: `Needs approval · In flight · History`** (V27). *Needs approval* is the admin approval gate (§7) with per-row review/edit and **bulk approve**; *In flight* is the tracked titles below; *History* is the decided proposals — each carrying **when** it was approved (`approvedAt`), who by, what they changed, and any deny reason. This is what "the approvals queue as its own surface" means: **Queue is that surface**, which is why the mock hangs a pending-proposal count badge off this nav entry and not a separate one. Tab counts come from the same queries that fill the tabs, so a count cannot disagree with its list. Tracked titles by provisioning state keep their **retry** on a stalled acquisition, and each title's journey (*pending approval → acquiring (3/7) → live on channel N*). Named for what it holds — work waiting on someone — rather than "Board", which named a layout. Per §342's global-read model this list is **not per-member**: every authenticated user sees every tracked title, because `TitleDTO` carries no requester and the store has no per-user index to scope by. "My requests" as a heading would therefore be a promise the data cannot keep — scoping it is a schema change (a requester column + a filtered list route), not a UI filter.
  ...
  **The member-facing "My requests" surface lists PROPOSAL JOBS, not titles and not only successful Proposals** (§7/§8). The two halves of a request scope differently: a job carries `created_by`, so *"the executions I submitted"* is answerable even while one is running or after it fails without a Proposal; a `Title` has no requester, so *"the titles acquired for me"* is not. When the job projection includes a Proposal, the row continues through submitted → approved/denied and shows the approver's **note**, server-generated **modification summary**, or **deny reason** — the provenance V25/V25b persist. Queue **History** remains Proposal-based because it audits decisions, while My requests audits executions. Tracked titles remain the global list above. **Cancel** is likewise unbuilt: giving up on a title is `DELETE /v1/titles/{key}`, which is admin-only by the same §342 rule that makes enqueuing admin-only, so it is not a member-facing control on this route.
...
3. **Connection checklist** — live-tests each dependency and shows pass/fail with a fix hint and a deep link into the relevant docs page: media server reachable + `library.token` valid; filler library found (if configured); Seerr reachable + key valid; LLM configured/reachable with the model present and its **tool capability shown separately** (Ollama: query declared capabilities; hosted metadata absence stays visibly `unverified` and points to the explicit Verify action); TMDB key valid. Checklist polling never invokes LLM inference. The LLM check can project connectivity and grounding capability evidence, **not semantic curation quality**; exact-template certification is a separate explicit, potentially paid action (§8.1), never part of checklist polling. **Tunarr is never a block on this step** — it is configured on the Playout step above — but on the Tunarr path its check still **gates** here: being configured elsewhere does not stop it being required (**Tunarr reachable *and* with a media source matching `library.url`**, queryable via Tunarr's API, which verifies §6's "Important" invariant instead of just documenting it).
...
6. **Guided first channel** — offer a template Intent (below); since the owner is an admin, they can self-approve and watch the full pipeline run end to end. Submission moves through durable states rather than a page-local spinner: proposal job → reviewable Proposal → approval → `building` Channel → `live` Guide/Watch. `building` means the local Channel exists and reconciliation will retry, **not** that a stream is ready; Watch makes no HLS request until the authoritative Channel read says `live`. Channel SSE only accelerates that refetch. Filler is optional throughout: an empty eligible filler pool produces a program-only Channel with back-to-back programs and never disables approval.
...
### Member first-run
- **First login intro** — one screen with the mental model: *Intent → proposal job → Proposal → admin approves → titles are acquired → `building` Channel → live in Guide/Watch.* Sets the expectation that progress and failure survive reload, that a member waits for admin approval, and that channels improve as content lands (§9's backfill).
- **Channel templates** — the blank-page killer: a set of one-click starter Intents ("90s Saturday Morning Cartoons," "Cozy Mystery Nights," "Late-Night Sci-Fi," "Action Movie Marathon") that prefill the suggestion workspace with a good Intent + sensible constraints. They live in **one canonical product-data source** consumed by the wizard, Guide, and semantic-evaluation harness. Each has a stable id and the complete typed Intent — description plus era/tone/must-include/exclude/runtime fields when present — so a handoff never collapses a preset to display copy. Users may edit before running; the untouched path submits the exact canonical object that certification evaluates.
...
- **"My requests" status** — proposal jobs preserve every accepted execution, including `queued`, `running`, and a failure with no Proposal. Once a Proposal exists the same journey continues through *pending approval → approved → acquiring (3/7 titles) → building → live on channel 42*. Failure keeps the complete Intent and offers Retry (a fresh job) or Edit; reloading or losing SSE cannot erase it. Approval history remains Proposal-based because it records decisions, not executions.
