...
| GET | `/v1/setup/status` | Run the connection checklist; structured pass/fail per integration (admin; powers the wizard + Settings troubleshooting, §13). Includes the `livetv` check ("Tunarr wired as tuner + guide in the media server") and the `tunarr_library` check ("Tunarr's Emby/Jellyfin source is wired + scanned" — §6). |
