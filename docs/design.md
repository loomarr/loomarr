...
The feed includes operator-relevant user-management transitions (`kind: user`) — account creation,
role/quota/access changes, password resets, and external-account refreshes — but never password,
session-token, or secret material. `subject_id` carries the affected user id when one exists.
...
| PATCH | `/v1/users/{id}` | Role / quotas / disable (admin). A quota is `0` (inherit the configured default) or positive; negative values are rejected. The current session cannot demote or disable itself — losing the admin surface must be a deliberate action by another admin, not a control that strands the operator who is using it. Returns the same usage-enriched body as `GET /v1/users`, including the effective quota and pending count. |
...
| GET | `/v1/dashboard/summary` | The Dashboard's compact operational read model (admin, §12): on-air/total channels, submitted approvals, actively acquiring titles, unavailable titles, and the instant the snapshot was generated. Counts are computed by the store rather than by downloading full channel/title/proposal collections into the browser. |
...
**Admin management safety.** A quota is either `0` (inherit) or a positive cap; a negative cap has no domain meaning and is rejected rather than silently behaving like the default. An authenticated admin may not demote or disable their own row through the People API. This is intentionally enforced at the backend as well as expressed in the UI: removing the authority or access of the session performing the mutation can strand the operator mid-action, while a second admin can still make the same deliberate change. Every successful user mutation returns the same usage-enriched representation as the list, so `effectiveQuota` and the displayed pending count cannot temporarily fall back to zero after a save. The representation also carries whether acquisition usage was read successfully; a failed accounting read renders as unavailable and must never masquerade as zero pending.
...
- **Dashboard** (admin, route `/dashboard`, V16) — *"is everything alright?"* Four stat cards (on air · needs you · acquiring · unavailable), each a link to the surface it summarizes, over a **Transcoding** panel showing live internal-playout telemetry: one row per encoding channel with its viewer count, resolved encoder (hardware vs software — the difference between four concurrent streams and one), realtime speed and buffer-ahead, plus an `active / capacity` load line against `playout.max_channels`. "Acquiring" is explicitly the active `wanted`/`requested`/`downloading` set; the terminal `unavailable` state is a separate attention count and must never inflate work in flight. The cards read `GET /v1/dashboard/summary`, a purpose-built store projection, rather than transferring full resource collections and counting them in the browser.

  Data comes from `GET /v1/playout/status` (admin-only), with the **`playout` SSE frame as the latency path**: the frame fires when a channel starts or stops, and the dashboard re-reads the endpoint. While internal playout is active the browser also polls that compact status every 5s so speed and buffer telemetry continue to move without publishing per-progress SSE frames — those arrive about once a second per stream, and republishing each would push several frames a second at every open browser for numbers that move by fractions. This is §8's standing rule applied: SSE is a latency optimization, the GET is truth on reconnect.

  Below it sit two more panels. **Services** (V31) lists every configured integration with the target it was probed against and a pass/fail state, **polled every 30s**, with a *"Fix →"* on a failing row that routes to the settings block owning it — a red dot that does not say where to go is a puzzle, not a diagnosis. Targets identify the endpoint but strip URL userinfo, queries, and fragments so a diagnostic response cannot echo embedded credentials. It runs the same `runConnectionChecks` as the wizard checklist and `/v1/system/reload`; a test asserts the single implementation, because three surfaces disagreeing about whether Emby is reachable is worse than any one of them being wrong.

  ⚠ **Services POLLS and the feed has a slower fallback, and that asymmetry is deliberate.** A probe result is not a state change anyone observes — the server only learns it by making six outbound calls (~730ms measured against a real stack). Pushing it over SSE would still need a server-side timer doing exactly that polling, but *continuously, whether or not any browser is open*; client polling stops when nobody is looking. It would also invert §8's rule: a pushed frame is safe to drop precisely because a GET can re-derive the truth, and here the probe **is** the truth — there is nothing to fall back to. The activity feed is the opposite shape (the server knows at write time, and `GET /v1/activity` is authoritative on reconnect), so it takes an `activity` frame for low latency and a 60s visible-page poll only as a recovery net. **Recent activity** (V32) is the persisted `activity` feed (§5) — *what Loomarr did*, newest first. ⚠ It reads the table, **not** the SSE bus: the bus is deliberately lossy, so a feed built on it would drop rows precisely when the system is busiest and most worth watching.

  **Freshness is hybrid, never SSE-only.** Every EventSource `open` — including a reconnect — invalidates the GET-backed views represented by event frames, because frames missed during the gap cannot be replayed. Dashboard summary and activity have a 60s visible-page fallback, services keep their 30s probe interval, active playout uses the 5s compact poll above, and window focus revalidates the page. Loading and failed reads render as unknown/error states, never as a trustworthy-looking zero.
...
- **People** (admin, route `/people`) — imported users, roles, quotas, disable, sync-now (§11). "People" rather than "Users" because the list is households and family members, not system accounts. The default view is a readable roster; account creation/import and session/password/danger actions are progressively disclosed instead of making every row a wall of controls. Credential copy describes the capability (Loomarr password vs external sign-in), not a guessed provider — an SSO allowlist row and a media-server row can both have `local: false`. Destructive actions require confirmation and report their result. The roster revalidates on window focus and on a slow visible-page interval so a second admin or periodic sync cannot leave it indefinitely stale.
