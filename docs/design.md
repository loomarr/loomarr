...
- **Backup (§16, SQLite only).** A **`backup`** job (default `0 30 3 * * *`) writes one `VACUUM INTO` snapshot into `backup.dir` and then prunes that directory to the newest `backup.retain` files, matching only the `loomarr-<timestamp>.db` names it writes. Prune runs **after** a successful write, so a failed snapshot never costs the operator a backup they already had. **Not registered on Postgres** — `WriteBackup` is SQLite-only, so registering it there would put a permanently failing row on the Tasks page for a backup strategy (`pg_dump`) the operator is correctly running elsewhere.
