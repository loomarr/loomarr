...
| GET | `/v1/filler/splits/{proposalId}` | Read a split proposal (admin, §10 V34) — the source of truth on SSE reconnect, the same pattern as `/v1/suggestions/{id}`. |
...
Each filler source — the drop-folder, the media-server library, and each remote collection — can be **disabled**. A disabled source is not scanned, not searched, and not downloaded from.

⚠ **Clips already in the catalog stay.** Disabling a source withdraws it from *future* work; it is not a delete, and it must never look like one. This is enforced at the scan, discovery and ingest sites rather than by hiding the source in the UI — a toggle that only dims a row is a claim the system does not honour.

The folder and library rows stay **derived from configuration** (the read-model decision above), so their switch is a setting; remote collections are rows, so theirs is a column. That asymmetry is deliberate and is why one source never appears twice.
...
6. **Review — not optional.** Because detection quality is a property of the source, the operator confirms the proposed cuts before anything enters the catalog; auto-accepting a 69% result puts 3-minute "commercials" into 30-second breaks. Detection runs as a **job** (minutes per file) producing a **persisted split proposal** (§5) — review can happen long after detection, and a restart must not lose it — and nothing writes to the catalog until `POST /v1/filler/splits/{id}/confirm` (§7) commits the operator's edited cut list.
...
| `FILLER_SOURCE_FOLDER_ENABLED` / `FILLER_SOURCE_LIBRARY_ENABLED` | `true` / `true` (§10 V35). The on/off switch for the two sources that are **derived from configuration** rather than stored as rows — a remote collection's switch is a column on its own row. Disabling withdraws a source from scanning, searching and downloading; ⚠ **it never removes clips already in the catalog**, and the enforcement lives at those three sites, not in the UI |
