...
- **System probe** (`GET /v1/system/llm`, admin) returns the active provider + model and **two curated catalogs**:
  ...
  - **Hosted providers** — a curated list (OpenRouter/OpenAI/Anthropic/Groq/Gemini). The **providers** (label, OpenAI-compatible base URL, keys-URL) are stable curated truth. The **model list**, however, is fetched **live** from each configured provider's own `{baseURL}/models` endpoint (the same call used for key validation), because hosted model ids churn constantly — a static list rots. Loomarr's curated model entries become **annotations** (the one-line cost/why hint + "recommended" flag) overlaid onto the live ids we recognize; the curated list is the **fallback** shown when no key is set / the provider is unreachable. Each provider carries a **`keyConfigured`** flag; the API key is **never echoed back**. A hosted provider needs no hardware fit — its readiness is *key present (and, on demand, valid)*. The user may always select an id they type, not only a recommended one.

  The **local** catalog is fully curated in-code (deterministic VRAM fit). The **hosted** provider list is curated but its **models are live** (current ids + curated hints). Neither is a blind registry scrape.
