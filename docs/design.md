...
Availability under the direct requester comes from the same **library scan** (§4, §18.1) as Seerr, plus the arr **queue poller** (progress + `Grabbed`) — never an inbound webhook.
...
- **Channels** — each channel's health, desired-vs-live status, "Managed by Loomarr" badge (§9 ownership), drift flags from slot revalidation, filler policy. A channel is **editable in place** (§7 `PATCH`): rename/renumber, per-channel programming rules (as editable chips, `programming-design.md`), manual lineup tweaks, **its filler** (§10), and **pause/resume + delete** (a typed-confirm danger zone). Edits apply **seamlessly** — no "rebuild now" button: an edit auto-reconciles in the background and the page updates live (the `channel` SSE frame). A channel can also be **refined with the LLM** here (§8: describe a change → review the diff → apply), so shaping an existing channel uses the same describe→review→approve loop that created it. **Filler is chosen and previewed here too:** a Filler section lets an admin narrow the channel's ad breaks by theme (era/audience/category/kinds) and pin/exclude specific clips, with a **live sandbox** — every change re-assembles the actual break against an unsaved draft (`POST …/pods/preview`, §7/§10) so you see exactly what will air before you **Apply** it. The pod preview that used to hide inside "Advanced" lives here now.
