...
- **System probe** (`GET /v1/system/llm`, admin) returns the active provider + model and **two curated catalogs**:
  ...
  - **Hosted providers** — only the **providers** (label, OpenAI-compatible base URL, keys-URL, note) are curated in-code; those are stable. The **model list is live** (`{baseURL}/models`), and recommendations rank it for **Loomarr's use case — best grounded tool-caller, not merely cheapest**. Ranking, over the live list:
...
### Views
- **Login** — local or imported-media-server credentials (§11); first-run flows into the **setup wizard** (§13): create the owning admin (bootstrap) → connection checklist → webhook handshake → import media-server users → guided first channel.
