...
- **Guide** (route `/guide`) — headed **"Channels"**, this is the single channels surface: a cross-channel time grid answering both *"what do I have"* and *"what is on"*. **Origination** (how a channel is born) is a header action on this surface: the everyday door is **"✦ Add a channel" → describe it** (the §13 describe→review→approve flow, inlined below the header), and an empty install shows the **"Dead air"** state whose one action opens the same panel. **Evolution** (shaping a live channel) happens on the detail page (`/channels/{id}`) and never re-originates it.
  ...
  **Channel surface map** (the guardrail against capability-without-UI — a PR adding a channel capability updates this table):
  ...
  | scope: genres, collections | `policy.scope.genres`, `.collections` | **none — ORPHANED**, but the two are in *different* states and the row used to conflate them. `genres` **binds**: the scheduler's scope pass filters on it (`schedule/slotting.go`), it is simply only settable inside a curation rule. ⚠ **`collections` is INERT at every layer** — it round-trips through `PATCH` and the policy blob, but no filter reads it and there is no library-adapter support for listing collections or resolving membership. Pinned honestly by `TestEnforce_CollectionsDoesNotBindYet`, which asserts a collection scope filters *nothing* and fails the moment that changes. The missing endpoint is the *second* prerequisite; the first is the engine filter it would feed. `scripts/capture-collections.sh` pins the media-server contract that decides both (whether members carry `ProviderIds` is the difference between a set-membership test and an N+1 on the scheduling path). | admin |
