...
| File | Viewport | Screens |
| --- | --- | --- |
| `loomarr-prototype-desktop-v2.dc.html` | 1280×800 | Login · Wizard (6 steps, **Database first**) · **Dashboard** · **Guide** (time-grid) · Channel detail · My requests · Suggest · Approvals · Filler (Catalog / **Incoming** / Sources) · **People** · Settings (Connections · AI · Defaults · **System** · **Security** · **All settings**) · **Account** · Help · Member intro · User drawer · Toast |
| `loomarr-prototype-mobile-v2.dc.html` | 375×812 | Dashboard · Guide · Queue · People · Settings · Help · Account |

✅ **The stale-Filler warning that stood here is RESOLVED (2026-08-02).** The maintainer export
landed: `loomarr-prototype-desktop-v2.dc.html` is now **632,110 bytes** and carries the redesigned
Filler screen (4 tabs → 3, a new Incoming tab, an approval-gated "pull") plus the Approvals
`FILLER PULL` card. It supersedes the 502,509-byte copy that rendered the old screen.

⚠ **Two corrections the export forced, both worth keeping.**

1. **`support.js` was NOT byte-identical, though this file and `FILLER-DELTA-2026-08-01.md` both
   said it was.** It went 64,222 → 69,150 bytes (155 changed lines) and is updated here. The
   original claim was not careless — it was checked — but it was checked against the **truncated**
   fetch, and a size/hash taken from a capped read describes the cap, not the file. **A
   verification performed against a known-truncated source carries the truncation into its own
   conclusion.** Re-verify anything asserted from a partial fetch once the full one lands.
2. **The values behind `poolStats` / `asks` / `reels` / `services` / `autoConfChips` are no longer
   unknown.** The delta doc's "do not invent them" instruction was correct while the JS was
   unreachable; the export includes all ~270 KB of it, so they are now readable at source. The
   instruction is retired, not relaxed.

`Loomarr Prototype v2.dc.html` and `… v2 copy.dc.html` in the export archive are **not** duplicates
any more: the unsuffixed file is the new export, the `copy` is the previous 502 KB snapshot. (In the
2026-07-24 import they were identical, which is what the superseded note here described.)
