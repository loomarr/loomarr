...
func validCommandInventory(snapshot time.Time, authority, host string) fillercorpus.Inventory {
	...
	representation := fillercorpus.InventoryRepresentation{Transport: fillercorpus.TransportHTTPS, Name: "clip.mp4", URL: "https://" + host + "/clip.mp4", MIMEType: "video/mp4", Bytes: 100}
	...
	return fillercorpus.Inventory{SchemaVersion: fillercorpus.InventorySchemaVersion, SnapshotAt: snapshot, Captures: []fillercorpus.Capture{{CaptureID: captureID, Transport: fillercorpus.TransportHTTPS, Authority: authority, RoleHint: role, SnapshotAt: snapshot, MaxRequests: 1, RequestsUsed: 1, MaxResponseBytes: 100, ResponseBytes: 50, MaxPredictedMediaBytes: 100, PredictedMediaBytes: 100, MaxWallTimeMS: 1000, WallTimeMS: 10}}, Cases: []fillercorpus.InventoryCase{{CaseID: fillercorpus.CaseID(authority, "clip"), CaptureIDs: []string{captureID}, Authority: authority, ItemID: "clip", Title: "Clip", RoleHints: []string{role}, RightsAssertions: []string{"review"}, ItemURL: "https://example.org/item", MetadataURL: "https://example.org/metadata", MetadataRetrievedAt: snapshot, MetadataSHA256: metadataSHA256, AllowedMediaHosts: []string{host}, Representation: representation}}}
}
