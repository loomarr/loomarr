...
func run(ctx context.Context, args []string, getenv func(string) string, stdout, stderr io.Writer, factory providerFactory) int {
	flags := flag.NewFlagSet("channel-recommend-cert", flag.ContinueOnError)
	...
	jsonBlob, err := json.MarshalIndent(card, "", "  ")
	if err != nil {
		return commandError(stderr, "encode scorecard", err)
	}
	jsonBlob = append(jsonBlob, '\n')
	summary := recommend.HumanSummary(card)
	if err := writeArtifact(*outPath, jsonBlob); err != nil {
		return commandError(stderr, "write machine scorecard", err)
	}
	if err := writeArtifact(*summaryPath, []byte(summary)); err != nil {
		return commandError(stderr, "write human summary", err)
	}
...
func newProvider(config providerConfig) (llm.Provider, error) {
	...
	case "openrouter":
		if config.BaseURL != openRouterAPIBase {
			return nil, fmt.Errorf("OpenRouter certification requires canonical API base %s", openRouterAPIBase)
		}
...
func writeArtifact(path string, blob []byte) error {
	...
	temporary, err := os.CreateTemp(dir, ".channel-recommend-*")
	if err != nil {
		return err
	}
...
func commandUsageError(stderr io.Writer, operation string, err error) int {
	_, _ = fmt.Fprintf(stderr, "channel-recommend-cert: %s: %v\n", operation, err)
	return 2
}
