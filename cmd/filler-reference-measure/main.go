...
func run(args []string, stdout, stderr io.Writer) int {
	...
	families, err := fillerbakeoffio.ReadStrictJSON[fillerreference.FamilyAudit](*familyPath)
	if err != nil {
		return fail(stderr, fmt.Errorf("families: %w", err))
	}
	seed, err := fillerbakeoffio.ReadStrictJSON[fillerreference.InspectionSeed](*seedPath)
	if err != nil {
		return fail(stderr, fmt.Errorf("seed: %w", err))
	}
	inputs := fillerreference.MeasurementInputIdentity{
		AuditSHA256: fillerreference.SHA256(auditRaw), FamilySHA256: fillerreference.SHA256(familyRaw), SeedSHA256: fillerreference.SHA256(seedRaw),
	}
	cases, err := fillerreference.InspectionCases(seed, audit, families, inputs)
	if err != nil {
		return fail(stderr, err)
	}
	tools := mediatools.NewFFmpegTools(*ffmpegPath, *ffprobePath, "", "", "")
	artifact := fillerreference.MeasurementArtifact{SchemaVersion: 1, GeneratedAt: generatedAt.UTC(), Inputs: inputs, Cases: make([]fillerreference.InspectionMeasurement, 0, len(cases))}
