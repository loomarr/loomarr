...
type caseIDs map[string]struct{}

func (c caseIDs) String() string { return "case IDs" }
func (c caseIDs) Set(value string) error {
	if strings.TrimSpace(value) == "" {
		return fmt.Errorf("case ID is empty")
	}
	c[value] = struct{}{}
	return nil
}
...
func run(args []string, stdout, stderr io.Writer) int {
	...
	selected := caseIDs{}
	flags.Var(selected, "case", "optional case ID to include; repeat to select several")
	...
	for _, item := range audit.Cases {
		...
		if len(selected) > 0 {
			if _, ok := selected[item.CaseID]; !ok {
				continue
			}
			seenSelected[item.CaseID] = struct{}{}
		}
	...
	if len(selected) > 0 && len(seenSelected) != len(selected) {
		return fail(stderr, fmt.Errorf("one or more selected cases are absent or excluded"))
	}
	result := fillerreference.BuildFamilyAudit(fillerreference.SHA256(auditRaw), fingerprints, generatedAt)
