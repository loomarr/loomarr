...
// The app's menu primitive (a thin themed wrapper over Radix). What is worth pinning is that the
// wrapper preserves the behaviour it exists to theme: the content is closed until the trigger opens
// it (and is portalled), a CheckboxItem carries its checked state as accessible ARIA, and a chosen
// item fires onCheckedChange. The animation classes are visual-baseline territory, not asserted here.
...
describe("DropdownMenu", () => {
  it("stays closed until the trigger is activated", async () => {
    ...
    expect(screen.getByRole("menuitemcheckbox", { name: "English" })).toBeInTheDocument();
  });
  ...
  it("reflects each item's checked state as ARIA", async () => {
    ...
    expect(screen.getByRole("menuitemcheckbox", { name: "English" })).toHaveAttribute("aria-checked", "true");
