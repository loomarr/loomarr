import type { ClientObservation } from "@loomarr/core/client-diagnostics";
...
interface BrowserHlsPlayerOptions {
  ...
  recordDiagnostic: (observation: ClientObservation) => void;
}
...
function useBrowserHlsPlayer({
  ...
  const [state, setState] = useState<{ channelId: string; status: BrowserPlayerStatus; error?: string }>({
    channelId,
    status: "idle",
  });
  ...
  const attach = useCallback(
    (video: HTMLVideoElement): (() => void) => {
      ...
      source
        .then((body) => {
          if (!current()) return;
          // The source port already prefers the relative same-origin form, which avoids
