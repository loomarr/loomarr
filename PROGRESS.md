...
**Next up:** the `scope.collections` **picker UI**, plus the one live check the backend is
carrying as a stated assumption (below). The backend shipped — see the row directly under this
paragraph — so the audit's three leftovers are now all closed at the engine level.
