...
and the v2 mock puts it first in the admin nav), or the Settings track (**V9** IA → **V12**
Backup/About). Audit leftovers: `scope.collections` is blocked on a **missing endpoint** (nothing
lists media-server collections), plus `policy.window` and the draft preview.
