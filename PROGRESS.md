...
**Next up:** nothing is claimed here. `scope.collections` shipped end to end (engine filter →
list endpoint → picker), which closes **all three** of the surface audit's leftovers. The one
outstanding item is a live check, not a build: the `ParentId` membership query below.
...
⚠ **One assumption is still unverified against a real server:** `ParentId=<boxset>` as the
membership query. It is the documented behaviour, but a BoxSet is not a folder in the library
tree the way a season sits under a series, and this repo has been wrong from memory twice before
(Live TV `Path` vs `Url`; the guide-refresh task id). `scripts/capture-collections.sh` pins it in
one read-only run. It fails LOUDLY if wrong (an empty member list, so an empty picker), never as
a silent mis-filter — which is why the backend shipped ahead of the check rather than waiting.
