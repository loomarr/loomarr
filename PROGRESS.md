...
**Active phase: 10 — Scheduler + Tunarr (the point of the app).** Application code complete and
gated (see the phase table); one maintainer-supervised follow-up remains: the live `/LiveTv/*`
capture (pin real payloads + guide-refresh task id from Emby via Tailscale, §6/§21). Phases 0–9 done.
Phase 10 built against the vendored Tunarr spec + dev-Tunarr (`tunarr-dev`, wired to Emby);
**server-assigns-channel-id** finding (Phase 0) honored in the Programmer adapter.
Follow-ups: (a) live Live TV capture → `internal/testkit/fixtures/livetv/`; (b) Sonarr
`import_webhook.json` fixture still to be captured (28GB re-download in flight; Sonarr webhook conn
id 3 left up to catch it — remove after). Phase-0 findings:
...
| 10 · Scheduler + Tunarr | code done (gate green); live capture pending | `make check` green (`-race`, lint 0) + `make test-pg` (channel round-trip/list/delete + **ClaimDueChannels concurrent** under real `FOR UPDATE SKIP LOCKED`) + `openapi-verify` deterministic; app boots with the channel scheduler + sweep, `/readyz` ok, clean shutdown. **Gate tests:** `go test ./internal/{schedule,programmer,channels,setup,api}/` — pure desired-lineup (3 strategies, deterministic shuffle, pending policy); Tunarr adapter server-assigns-id + slot↔lineup + 400-on-empty vs pinned fixtures; **reconcile create→idempotent-no-op, backfill-on-event, event-loss-recovery-via-sweep, drift-substitution, guide-poke-only-when-affecting**; **idempotent livetv-connect second-call-no-op** (unit + over-the-wire); §19 auth negatives (member 403 on create/reconcile/connect/status) | `internal/{schedule,programmer,channels,setup}` + `store` channels (migration `00003`, `ClaimDueChannels` per-channel lease §18) + `library` Live TV wiring + `/v1/channels*` + `/v1/setup/{status,livetv-connect}`. Server-assigns-channel-id honored. **Fixed a pre-existing exporter spec-drift gap** (auth/users routes were never in the committed spec). Added `TUNARR_TRANSCODE_CONFIG_ID` to §15 doc-first. **FOLLOW-UP (maintainer-supervised, §21):** live `/LiveTv/TunerHosts`+`/LiveTv/ListingProviders` payloads + guide-refresh task id capture from real Emby → `internal/testkit/fixtures/livetv/`; adapter currently uses doc-shaped payloads + a placeholder task id (marked version-fragile in `library/livetv.go`). |
