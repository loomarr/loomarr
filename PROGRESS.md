...
**Next up:** `scope.collections` — and it is the ONLY one of the three audit leftovers still
standing. It is blocked on a **missing endpoint**: nothing lists media-server collections, so
there is nothing to build a picker against. Backend work first, UI second.
