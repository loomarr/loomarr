...
is TMDB's trademark and this repository carries no brand assets, so `TmdbAttribution` takes it as a
prop and renders the notice alone. (2) `channel_icons.bytes` was the schema's ONLY BLOB/BYTEA
column, so dropping the table left `copyTable`'s binary-coercion branch reachable by nothing and
`TestIconBytesSurviveMigration` deleted rather than rewritten — it could only have been rewritten
against a synthetic table added to both dialects' migrations for a test's benefit. Keeping the
branch means the next binary column does not silently arrive mangled; the cost is an uncovered
path. Both are recorded in §22 and in `migrate_data.go`, not left as folklore.
