...
**Next up:** the Settings track (**V9** IA → **V12** Backup/About), **V31** (Dashboard Services
panel — reuses `POST /v1/setup/test`) or **V32** (activity feed), both now unblocked by V16's
shell. Audit leftovers: `scope.collections` is blocked on a **missing endpoint** (nothing lists
media-server collections), plus `policy.window` and the draft preview.
