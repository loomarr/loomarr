...
**Phase 10 — Scheduler + Tunarr (the point of the app) — COMPLETE.** Application code + the
maintainer-supervised live `/LiveTv/*` capture both done and gated (see the phase table). Phases
0–10 done; **next: Phase 11 (Suggester — grounding gate).** Phase 10 built against the vendored
Tunarr spec + dev-Tunarr (`tunarr-dev`, wired to Emby); **server-assigns-channel-id** finding
(Phase 0) honored. Remaining follow-up (non-blocking, carried from Phase 6): Sonarr
