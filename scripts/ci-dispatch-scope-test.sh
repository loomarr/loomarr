...
[[ "$($selector tuner)" == "$tuner_expected" ]] || {
	echo 'tuner scope drifted from its isolated assurance set' >&2
	exit 1
}
[[ "$($selector tuner impact | selected_gates)" == "$tuner_impact_expected" ]] || {
	echo 'tuner scope no longer selects the tuner gate alone' >&2
	exit 1
}
