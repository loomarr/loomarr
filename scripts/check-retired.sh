...
cd "$ROOT"
RETIRED=(
  'scores.overall|retired: proposal assessment reports evidence without a synthetic composite'
  'func composite(s Scores)|retired: the proposal composite is not an assessment contract'
  ...
  'filler.reject.unidentified|#1249: missing optional classification is descriptive and never rejects household filler'
	...
	'filler.fetch.max_disk_gb|#1240: use filler.storage.library_budget_gb; one host-aware governor replaces the folder-size fetch ceiling'
  'FILLER_FETCH_MAX_DISK_GB|#1240: use FILLER_STORAGE_LIBRARY_BUDGET_GB'
	'filler.ai_tagging|#1251: household text enrichment is automatic when the configured AI provider is available'
  'FILLER_AI_TAGGING|#1251: there is no second Filler-specific AI switch'
	'filler.language_provider|#1371: language detection and transcripts share asr.provider'
	'FILLER_LANGUAGE_PROVIDER|#1371: use ASR_PROVIDER for the one speech-recognition service'
	'filler.transcribe.provider|#1356: speech recognition is the app-wide asr.provider connection'
	'filler.transcribe.model|#1356: speech recognition is the app-wide asr.model connection'
	'FILLER_TRANSCRIBE_PROVIDER|#1356: use ASR_PROVIDER'
	'FILLER_TRANSCRIBE_MODEL|#1356: use ASR_MODEL'
	'AskAboutAudio|#1371: language detection uses the speech transcription endpoint, not audio chat'
	'type AudioRequest|#1371: audio chat was retired with the duplicate language-model path'
	'NewTagger|#1251: progressive per-axis enrichment replaced the whole-catalog tagging sweep'
	'NewTagStage|#1251: descriptive text classification no longer runs on the readiness conveyor'
	'StageTag|#1251: descriptive text classification no longer runs on the readiness conveyor'
	'OperationID: "tag-filler"|#1251: progressive enrichment is automatic background work'
	'Path: "/v1/filler/tag"|#1251: progressive enrichment is automatic background work'
  ...
  'PinClipDialog|#1274: Library is not a channel-assignment workflow; use advanced channel preferences'
  'ChannelOverridePicker|#1274: automatic selection is the default, not an unchecked include-set'
  'CycleChip|#1273: optional clip corrections require explicit Edit details and Save/Cancel'
  'ClipTagDialog|#1273: use the optional panel-hosted ClipDetailsEditor, not a standalone tagging chore'
