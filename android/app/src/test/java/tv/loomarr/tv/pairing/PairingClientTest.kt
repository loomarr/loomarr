...
class PairingClientTest {
    ...
    @Test
    fun `start returns the code to display and the poll interval`() = runTest {
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
                .setBody(
    ...
    @Test
    fun `start falls back to a default interval`() = runTest {
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
     ...
     */
    @Test
    fun `a pending pairing reports Pending, not an error`() = runTest {
    ...
    @Test
    fun `an approved pairing yields the durable token`() = runTest {
        server.enqueue(
            MockResponse()
                .setResponseCode(200)
    ...
    @Test
    fun `a dead code reports Expired so the device starts over`() = runTest {
    ...
    fun `an unexpected status raises`() = runTest {
